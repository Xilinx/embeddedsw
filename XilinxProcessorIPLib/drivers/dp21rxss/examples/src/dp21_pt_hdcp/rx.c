...
extern u32 infofifo[64]; //RX and TX can store upto 4 infoframes each. fifo of 8
extern u8 endindex;
extern u8 fifocount;
u32 hdrframe[9];
u16 fifoOverflow=0;
...
void DpRxSs_TrainingDoneHandler(void *InstancePtr)
{
	...
	ReadVal = XDp_ReadReg(TX_CLK_RST_ADDR,
		0x8);
	ReadVal = (ReadVal & 0x3);
	ReadVal = (ReadVal & 0x1); //Putting FIFO in reset
	XDp_WriteReg(TX_CLK_RST_ADDR,
		0x8, ReadVal);
	//
}
...
void DpRxSs_AccessErrorCounterHandler(void *InstancePtr)
{
	...
	XVphy_DrpRd(&VPhyInst, 0, XVPHY_CHANNEL_ID_CH1,
		    ...
		    &DrpRdScratch);
	DrpVal_lower_lane0 = DrpRdScratch;
	XVphy_DrpRd(&VPhyInst, 0, XVPHY_CHANNEL_ID_CH1,
	...
	XVphy_DrpRd(&VPhyInst, 0, XVPHY_CHANNEL_ID_CH2,
		    XVPHY_DRP_GTHE4_PRBS_ERR_CNTR_LOWER,
		    &DrpRdScratch);
	DrpVal_lower_lane1 = DrpRdScratch;
	XVphy_DrpRd(&VPhyInst, 0, XVPHY_CHANNEL_ID_CH2,
		    XVPHY_DRP_GTHE4_PRBS_ERR_CNTR_UPPER, &DrpRdScratch);
	DrpVal = DrpRdScratch;
	...
	XVphy_DrpRd(&VPhyInst, 0, XVPHY_CHANNEL_ID_CH3,
		    XVPHY_DRP_GTHE4_PRBS_ERR_CNTR_LOWER,
		    &DrpRdScratch);
	DrpVal_lower_lane2 = DrpRdScratch;
	XVphy_DrpRd(&VPhyInst, 0, XVPHY_CHANNEL_ID_CH3,
		    XVPHY_DRP_GTHE4_PRBS_ERR_CNTR_UPPER, &DrpRdScratch);
	DrpVal = DrpRdScratch;
	...
	XVphy_DrpRd(&VPhyInst, 0, XVPHY_CHANNEL_ID_CH4,
		    XVPHY_DRP_GTHE4_PRBS_ERR_CNTR_LOWER,
		    &DrpRdScratch);
	DrpVal_lower_lane3 = DrpRdScratch;
	XVphy_DrpRd(&VPhyInst, 0, XVPHY_CHANNEL_ID_CH4,
		    XVPHY_DRP_GTHE4_PRBS_ERR_CNTR_UPPER, &DrpRdScratch);
	DrpVal = DrpRdScratch;
