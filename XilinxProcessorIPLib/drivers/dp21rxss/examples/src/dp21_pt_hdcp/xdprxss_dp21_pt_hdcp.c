...
#if (ENABLE_HDCP1x_IN_TX | ENABLE_HDCP1x_IN_RX)
unsigned int gKeyMGMTBaseAddress[2] = {KEYMGMT_BASEADDR_RX, KEYMGMT_BASEADDR_TX};
...
extern XDp_MainStreamAttributes* Msa_Pt;


//DDR Addresses of Frame Buffer
#ifndef PLATFORM_MB
#define ADDR1 0x10000000
#define ADDR2 0x20000000
#define ADDR3 0x30000000
#define ADDR1_Y 0x40000000
#define ADDR2_Y 0x50000000
#define ADDR3_Y 0x60000000

#else

#define ADDR1 0x90000000
#define ADDR2 0xA0000000
#define ADDR3 0xB0000000
#define ADDR1_Y 0xC0000000
#define ADDR2_Y 0xD0000000
#define ADDR3_Y 0xE0000000

#endif

//Gap between two Frame Buffer locations
#define DELTA 0x10000000
u32 frame_array[3] = {ADDR1, ADDR2, ADDR3};
u32 frame_array_y[3] = {ADDR1_Y, ADDR2_Y, ADDR3_Y};
...
#ifdef PLATFORM_MB //VCU118 (270 MHz on REFCLK0, 400 MHz on REFCLK1)
...
#else //ZCU102 (270 MHz on REFCLK0, 400 MHz on NORTHREFCLK0)
XVphy_PllRefClkSelType VPHY_REFCLK_SEL_270 = XVPHY_REF_CLK_SEL_XPLL_GTREFCLK0;
...
int main()
{
	...
	xil_printf("\n******************************************************"
				"**********\n\r");
	xil_printf("            DisplayPort 2.x Pass Through Demonstration"
			"                \n\r");
	xil_printf("                   (c) by AMD   ");
	xil_printf("%s %s\n\r\r\n", __DATE__  ,__TIME__ );
	...
	xil_printf("                      DP SS : %d byte\r\n",	8);
#endif
	xil_printf("                      Max BPC : %d \r\n",	XPAR_DP_RX_HIER_0_V_DP_RXSS2_0_DP_MAX_BITS_PER_COLOR);
	xil_printf("\n********************************************************"
				"********\n\r");
...
#else
	DpSs_PhyInit(XPAR_VID_PHY_CONTROLLER_0_BASEADDR);
#endif
...
// #if (ENABLE_HDCP1x_IN_RX | ENABLE_HDCP1x_IN_TX)
...
#else
	Status = config_phy(&VPhyInst, XDP_TX_LINK_BW_SET_UHBR10, VPHY_TX_PLL_TYPE, VPHY_TX_CHANNEL_TYPE, XVPHY_DIR_TX);
#endif
	if (Status != XST_SUCCESS) {
	...
	Status = DpSs_SetupIntrSystem();
	if (Status != XST_SUCCESS) {
		xil_printf ("Interrupts Setup Failed\r\n");
	}
...
// IIC_BASE_ADDR
    //Closing Mux 0x75
    // Buffer[0] = 0x0;
	// ByteCount = XIic_Send(IicInstance.BaseAddress, I2C_MUX_ADDR,
	// 		(u8*)Buffer, 1, XIIC_STOP);
	// if (ByteCount != 1) {
	// 	xil_printf("Failed to set the I2C Mux.\n\r");
	//     return XST_FAILURE;
	...
	//Opening Mux 0x74
    Buffer[0] = 0x1;
	ByteCount = XIic_Send(IicInstance.BaseAddress, I2C_MUX_ADDR2,
			(u8*)Buffer, 1, XIIC_STOP);
	if (ByteCount != 1) {
		xil_printf("Failed to set the I2C Mux.\n\r");
	    return XST_FAILURE;
	...
	Si570_SetFreq (&IicInstance, IIC_SI570_ADDRESS, audio_clk_Hz);
...
//Audio Clock of 24.576 MHz
    ...
    // } else {
    //     xil_printf ("Programmed Si570 for 24.576 MHz..\r\n");
    // }
#else
//For ZCU102
	//Closing Mux 0x75
	...
	Si570_SetFreq (&Ps_Iic1, IIC_SI570_ADDRESS, audio_clk_Hz);
	//Closing the Mux 0x74
	I2cMux_Ps(0x0);
	//Opening Mux 0x75
    // Buffer[0] = 0x1;
	// ByteCount = XIic_Send(IicInstance.BaseAddress, I2C_MUX_ADDR,
	...
	//Issue reset after Aud clock is programmed
	ReadVal = XDp_ReadReg(TX_CLK_RST_ADDR, 0x8);
	ReadVal = (ReadVal & 0x7); //Maintaining values
	ReadVal = (ReadVal & 0x3); //Set bit[2] to 0
	XDp_WriteReg(TX_CLK_RST_ADDR, 0x8, ReadVal);

	ReadVal = XDp_ReadReg(TX_CLK_RST_ADDR, 0x8);
	ReadVal = (ReadVal & 0x7); //Maintaining values
	ReadVal = (ReadVal | 0x4); //Set bit[2] to 1
	XDp_WriteReg(TX_CLK_RST_ADDR, 0x8, ReadVal);
#endif
...
/**
*
* This function sets VPHY based on the link rate
*
...
*
******************************************************************************/
u32 config_phy (XVphy *InstancePtr, int LineRate, XVphy_PllType Pll,
...
#ifndef PLATFORM_MB
	//GTHE4 cannot support 20G
	if(LineRate == XDP_TX_LINK_BW_SET_UHBR20) {
		xil_printf ("GTHE4 does not support 20G... skipping\r\n");
		return Status;
	}
#endif
...
u32 DpSs_SetupIntrSystem(void)
{
	...
	/* Hook up Rx interrupt service routine */
	Status = XIntc_Connect(IntcInstPtr,
			XPAR_INTC_0_DP21RXSS_0_DPRXSS_TIMER_IRQ_VEC_ID,
...
#if ENABLE_HDCP_IN_DESIGN
#if (ENABLE_HDCP1x_IN_RX || ENABLE_HDCP22_IN_RX)
	XIntc_Enable(IntcInstPtr, XPAR_INTC_0_DP21RXSS_0_DPRXSS_TIMER_IRQ_VEC_ID);
#endif
#endif
...
u32 DpSs_SetupIntrSystem(void)
{
	...
	Status = XSetupInterruptSystem(&DpTxSsInst, XHdcp1x_CipherIntrHandler,
	...
	Status = XSetupInterruptSystem(&DpRxSsInst, XHdcp1x_CipherIntrHandler,
...
void frameBuffer_start_rd(XDpTxSs_MainStreamAttributes Msa[4]) {
	...
	u32 stride = CalcStride(Cfmt,
					...
					&VidStream);
	if(Clip_4k == 1){
		const XVidC_VideoTiming *ClipTimingPtr;
		VidStream.VmId = XVIDC_VM_3840x2160_30_P; //VmId; // This will be set as 4K30
		ClipTimingPtr = XVidC_GetTimingInfo(VidStream.VmId);
		VidStream.Timing = *ClipTimingPtr;
...
void bufferWr_callback(void *InstancePtr){
	...
	if(XVFRMBUFWR_BUFFER_BASEADDR >= (0 + (ADDR1) + (DELTA * 2))){

		XVFRMBUFRD_BUFFER_BASEADDR = (0 + (ADDR1) + (DELTA * 1) +
										offset_rd);
		XVFRMBUFRD_BUFFER_BASEADDR_Y = (0 + (ADDR1_Y) + (DELTA * 1) +
										offset_rd);

		XVFRMBUFWR_BUFFER_BASEADDR = 0 + (ADDR1);
		XVFRMBUFWR_BUFFER_BASEADDR_Y = 0 + (ADDR1_Y);
	}else{
		...
		XVFRMBUFWR_BUFFER_BASEADDR = XVFRMBUFWR_BUFFER_BASEADDR + DELTA;
		XVFRMBUFWR_BUFFER_BASEADDR_Y = XVFRMBUFWR_BUFFER_BASEADDR_Y + DELTA;
	}
...
int Dppt_DetectResolution(void *InstancePtr){//,
	(void)InstancePtr;
	char *color;
	Msa_Pt = XDpRxss_GetMsa(&DpRxSsInst);
	frameBuffer_stop_wr();
	XDp_RxSetLineReset(DpRxSsInst.DpPtr,XDP_TX_STREAM_ID1);
	XDp_RxDtgDis(DpRxSsInst.DpPtr);
	if(Msa_Pt[0].ComponentFormat == 0){
		color="RGB";
	}else if(Msa_Pt[0].ComponentFormat == 1){
		color="YUV422";
	}else if(Msa_Pt[0].ComponentFormat == 2){
		color="YUV444";
	}
	if (DpRxSsInst.link_up_trigger == 1) {
		xil_printf(
			"*** Resolution: "
				...
				Msa_Pt[0].Vtm.Timing.HActive, Msa_Pt[0].Vtm.Timing.VActive,Msa_Pt[0].Vtm.FrameRate,Msa_Pt[0].BitsPerColor,
				color);
	}
	if (DpRxSsInst.link_up_trigger == 1) {
#if XPAR_XV_FRMBUFWR_NUM_INSTANCES
		frameBuffer_start_wr(Msa_Pt);
#endif
...
int Dppt_DetectColor(void *InstancePtr,
	...
	if(color_mode == 0){
		...
		component = Msa_Pt[0].ComponentFormat;
		xil_printf(ANSI_COLOR_RED"Unsupported Color Format !!"ANSI_COLOR_RESET"\r\n");
	}

	if (component != Msa_Pt[0].ComponentFormat) {
			for (x=0;x<500;x++){
				for (y=0;y<500;y++){
		...
		if (DpRxSsInst.link_up_trigger == 1) {
			...
			Dppt_DetectResolution(InstancePtr);//, Msa_Pt);
			if (DpRxSsInst.link_up_trigger == 1) {
				return 1;
			}
			return 0;
