...
*
******************************************************************************/
void Dp21RxSs_Pt_Si570_SetFreq(void *IicPtr, u8 Si_Addr, double Freq)
{
	u8 RFreq_Cal[5];
	u8 HSDIV_Cal;
	u8 N1_Cal;
	 ...
	 */
	Dp21RxSs_Pt_Si570_ReadCal(IicPtr, Si_Addr, RFreq_Cal, &HSDIV_Cal, &N1_Cal);
	Dp21RxSs_Pt_Si570_XtalFreqCalc(&XtalFreq, RFreq_Cal, &HSDIV_Cal, &N1_Cal);

	/*
	 * Calculate New Frequency Settings
	 */
	Dp21RxSs_Pt_Si570_RfreqCalc(Freq, RFreq_Cal, RFreq_Set, &HSDIV_Set,	&N1_Set, &XtalFreq);

	/*
	 * Write New Frequency Settings
	 */
	Dp21RxSs_Pt_Si570_WriteRfreq(IicPtr, Si_Addr, RFreq_Set, HSDIV_Set, N1_Set);
}

void Dp21RxSs_Pt_Si570_ReadCal(void *IicPtr, u8 Addr, u8 *RFreq_Cal,
...
void Dp21RxSs_Pt_Si570_XtalFreqCalc(double *XtalFreq, u8 *RFreq_Cal,
                        u8 *HSDIV_Set, u8 *N1_Set)
{
	...
	*XtalFreq = (156.25 * HS_DIV * N1) / RFREQ;
}

void Dp21RxSs_Pt_Si570_RfreqCalc(double Freq, u8 *RFreq_Cal, u8 *RFreq_Set,
...
void Dp21RxSs_Pt_Si570_WriteRfreq(void *IicPtr, u8 Addr, u8 *RFreq_Set,
                        u8 HSDIV_Set, u8 N1_Set)
{
	int Status = XST_SUCCESS;
	u8 reg7,reg8;
	...
	do {
		 ...
		 * 137:Freeze DCO
		 */
		Status |= Si570_I2cSend(IicPtr, Addr, (u8[]){137,0x18}, 2, I2C_STOP);

		/*
		 * 7:HSDIV[2:0], N1[6:2]
		 */
		Status |= Si570_I2cSend(IicPtr, Addr, (u8[]){  7,reg7}, 2, I2C_STOP);

		/*
		 * 8:N1[1:0],RFREQ[37:32]
		 */
		Status |= Si570_I2cSend(IicPtr, Addr, (u8[]){  8,reg8}, 2, I2C_STOP);

		/*
		 * 9:RFREQ[31:24]
		 */
		Status |= Si570_I2cSend(IicPtr, Addr, (u8[]){  9,*(RFreq_Set+1)}, 2, I2C_STOP);

		/*
		 * 10:RFREQ[23:16]
		 */
		Status |= Si570_I2cSend(IicPtr, Addr, (u8[]){ 10,*(RFreq_Set+2)}, 2, I2C_STOP);

		/*
		 * 11:RFREQ[15:8]
		 */
		Status |= Si570_I2cSend(IicPtr, Addr, (u8[]){ 11,*(RFreq_Set+3)}, 2, I2C_STOP);

		/*
		 * 12:RFREQ[ 7:0]
		 */
		Status |= Si570_I2cSend(IicPtr, Addr, (u8[]){ 12,*(RFreq_Set+4)}, 2, I2C_STOP);

		/*
		 * 137:Unfreeze DCO
		 */
		Status |= Si570_I2cSend(IicPtr, Addr, (u8[]){137,0x08}, 2, I2C_STOP);

		/*
		 * 135:Reset
		 */
		Status |= Si570_I2cSend(IicPtr, Addr, (u8[]){135,0x40}, 2, I2C_STOP);
