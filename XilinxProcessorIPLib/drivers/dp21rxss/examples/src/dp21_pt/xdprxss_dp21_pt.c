...
/**
*
* @file xdprxss_dp21_pt.c
*
* This file contains a design example that uses both the dp21rxss and dp21txss
* drivers in Single Stream Transport (SST) mode to demonstrate a pass-through
* design.
*
...
void Dp21RxSs_Pt_ResetWrFb();
void Dp21RxSs_Pt_ResetRdFb();
void Dp21RxSs_Pt_TxMain();
void Dp21RxSs_Pt_Setup();
#ifndef SDT
u32 Dp21RxSs_Pt_PhyInit(u16 DeviceId);
#else
u32 Dp21RxSs_Pt_PhyInit(UINTPTR BaseAddress);
#endif
u32 Dp21RxSs_Pt_CalcStride(XVidC_ColorFormat Cfmt, u16 AXIMMDataWidth,
...
#define PS_IIC_CLK 100000
int Dp21RxSs_Pt_I2cMuxPs(u8 Data);
#endif
...
*
******************************************************************************/
int Dp21RxSs_Pt_VideoFMC_Init(void)
{
	...
	while(freq != 1) {
		freq = Dp21RxSs_Pt_I2cReadFreq (IIC_BASE_ADDR, 0x4D, 0x0);
	}
...
int Dp21RxSs_Pt_I2cWriteFreq(u32 I2CBaseAddress, u8 I2CSlaveAddress, u8 RegisterAddress, u32 Value)
{
	u32 Status;
	u32 ByteCount = 0;
	u8 Buffer[4];
...
u8 Dp21RxSs_Pt_I2cReadFreq(u32 I2CBaseAddress, u8 I2CSlaveAddress, u16 RegisterAddress)
{
	u32 ByteCount = 0;
	u8 Buffer[1];
	u8 Data;
...
int Dp21RxSs_Pt_I2cWriteTdp2004(u32 I2CBaseAddress, u8 I2CSlaveAddress,
...
u8 Dp21RxSs_Pt_I2cReadTdp2004(u32 I2CBaseAddress, u8 I2CSlaveAddress, u16 RegisterAddress)
{
	u32 ByteCount = 0;
	u8 Buffer[1];
	u8 Data;
...
static int Dp21RxSs_Pt_Init(void)
{
	u32 Status;
#ifdef Rxo
	...
	XHdcp22_LoadKeys_rx(Hdcp22Lc128, Hdcp22Lc128_Sz,
			    Hdcp22RxPrivateKey, Hdcp22RxPrivateKey_Sz);
	password_valid = 0;
#else
	 ...
	 */
	XV_DpRxSs_Hdcp22SetKey(&DpRxSsInst, XV_DPRXSS_KEY_HDCP22_LC128,
			       Hdcp22Lc128);
	XV_DpRxSs_Hdcp22SetKey(&DpRxSsInst, XV_DPRXSS_KEY_HDCP22_PRIVATE,
			       Hdcp22RxPrivateKey);
#ifndef USE_EEPROM_HDCP_KEYS
	/*
	 * Load HDCP22 Keys
	 ...
	 */
#ifdef PLATFORM_MB
	Status = Dp21RxSs_Pt_PhyConfig(&VPhyInst, XDP_TX_LINK_BW_SET_UHBR20, VPHY_TX_PLL_TYPE, VPHY_TX_CHANNEL_TYPE, XVPHY_DIR_TX);
#else
	Status = Dp21RxSs_Pt_PhyConfig(&VPhyInst, XDP_TX_LINK_BW_SET_UHBR10, VPHY_TX_PLL_TYPE, VPHY_TX_CHANNEL_TYPE, XVPHY_DIR_TX);
#endif
	if (Status != XST_SUCCESS) {
...
#ifdef Rxo
	Status = Dp21RxSs_Pt_PhyConfig(&VPhyInst, XDP_TX_LINK_BW_SET_UHBR10, VPHY_RX_PLL_TYPE, VPHY_RX_CHANNEL_TYPE, XVPHY_DIR_RX);
	if (Status != XST_SUCCESS) {
...
static int Dp21RxSs_Pt_ConfigTxAudioClock(u32 AudioClkHz)
{
#if ENABLE_AUDIO
	u32 ReadVal;
#ifdef PLATFORM_MB
	...
	Dp21RxSs_Pt_Si570_SetFreq (&IicInstance, IIC_SI570_ADDRESS, AudioClkHz);
	 ...
	 */
	Dp21RxSs_Pt_I2cMuxPs(0x04);
	Dp21RxSs_Pt_Si570_SetFreq (&Ps_Iic1, IIC_SI570_ADDRESS, AudioClkHz);
	/*
	 * Closing the Mux 0x74
	 */
	Dp21RxSs_Pt_I2cMuxPs(0x0);
#endif
	/*
...
* DpSs_Main function which setup the system succeeds, this function
...
* 		DpSs_Main is blocking (it is waiting on interrupts).
*
******************************************************************************/
int main()
{
	...
	xil_printf("                      DP RX SS : %d byte\r\n",	4);
#endif
#if XPAR_DP_RX_HIER_0_V_DP_RXSS2_0_DP_OCTA_PIXEL_ENABLE
	xil_printf("                      DP RX SS : %d byte\r\n",	8);
#endif
#if XPAR_DP_TX_HIER_0_V_DP_TXSS2_0_DP_QUAD_PIXEL_ENABLE
	xil_printf("                      DP TX SS : %d byte\r\n",	4);
#endif
#if XPAR_DP_TX_HIER_0_V_DP_TXSS2_0_DP_OCTA_PIXEL_ENABLE
	xil_printf("                      DP TX SS : %d byte\r\n",	8);
#endif

	xil_printf("                      Max BPC : %d byte\r\n",	XPAR_XDP_0_MAX_BITS_PER_COLOR);
	xil_printf("\n*********************************************************\n\r");

	Status = Dp21RxSs_Pt_Main();
	if (Status != XST_SUCCESS) {
...
u32 Dp21RxSs_Pt_Main(void)
{
	...
	Status = Dp21RxSs_Pt_PlatformInit();
	if (Status != XST_SUCCESS) {
	 ...
	 */
#ifndef SDT
	Dp21RxSs_Pt_PhyInit(XVPHY_DEVICE_ID);
#else
	Dp21RxSs_Pt_PhyInit(XPAR_VID_PHY_CONTROLLER_0_BASEADDR);
#endif

	Status = Dp21RxSs_Pt_Init();
	if (Status != XST_SUCCESS) {
		xil_printf("Pass-through init failed!\n\r");
		return XST_FAILURE;
	}
	xil_printf("Pass-through init done.\n\r");

	Status = Dp21RxSs_Pt_SetupIntrSystem();
	if (Status != XST_SUCCESS) {
		xil_printf ("Interrupts Setup Failed\r\n");
		return XST_FAILURE;
	}

	Status = Dp21RxSs_Pt_ConfigTxAudioClock(audio_clk_Hz);
	if (Status != XST_SUCCESS) {
		xil_printf("Audio config failed!\n\r");
	}
...
void Dp21RxSs_Pt_PLLRefClkSel (XVphy *InstancePtr, u8 link_rate, XVphy_ChannelId Channel)
{
	XVphy_CfgQuadRefClkFreq(InstancePtr, 0, VPHY_REFCLK_SEL_270, 270000000);
	XVphy_CfgQuadRefClkFreq(InstancePtr, 0, VPHY_REFCLK_SEL_400, 400000000);
...
*
******************************************************************************/
u32 Dp21RxSs_Pt_PhyConfig(XVphy *InstancePtr, int LineRate, XVphy_PllType Pll,
	       XVphy_ChannelId Channel, XVphy_DirectionType Dir)
{
	...
	Dp21RxSs_Pt_PLLRefClkSel(InstancePtr, LineRate, Channel);
...
******************************************************************************/
#ifdef Rxo
void Dp21RxSs_Pt_CalculateCRC(void)
{
	u8 color_mode = 0;
	...
	XVidFrameCrc_SetVideoFormat(&VidFrameCRC_rx, VidFrameCRC_rx.Mode_422, CRC_CFG_RX);
}
#endif
...
*
******************************************************************************/
void Dp21RxSs_Pt_I2CScan(u32 BaseAddress)
{
	u8 Buffer[2];
	int BytesRecvd;
	int i;
...
*
******************************************************************************/
u32 Dp21RxSs_Pt_PlatformInit(void)
{
	u32 Status = XST_SUCCESS;
	...
	XVidFrameCrc_Initialize(&VidFrameCRC_rx, CRC_CFG_RX);
#endif

#ifdef Txo
	VidFrameCRC_tx.Base_Addr = VIDEO_FRAME_CRC_TX_BASEADDR;
	XVidFrameCrc_Initialize(&VidFrameCRC_tx, CRC_CFG_TX);
#endif
	...
	Dp21RxSs_Pt_VideoFMC_Init();

	/*
	 * dummy read
	 */
	(void)Dp21RxSs_Pt_I2cReadTdp2004(IIC_BASE_ADDR, 0x18, 0xF0);
	(void)Dp21RxSs_Pt_I2cReadTdp2004(IIC_BASE_ADDR, 0x18, 0xF1);

	/*
	 * Setting the gain to 2.6db
	 */
	Dp21RxSs_Pt_I2cWriteTdp2004(IIC_BASE_ADDR, 0x18, 0x83, 0x7);

	/*
	 * putting the driver in run mode
	 */
	Dp21RxSs_Pt_I2cWriteTdp2004(IIC_BASE_ADDR, 0x18, 0x84, 0x4);
...
******************************************************************************/
#ifndef SDT
u32 Dp21RxSs_Pt_SetupIntrSystem(void)
{
	u32 Status;
	 ...
	 */
#ifdef Txo
	Dp21RxSs_Pt_TxSetupIntrSystem();
#endif
	/*
	 * Rx side
	 */
#ifdef Rxo
	Dp21RxSs_Pt_RxSetupIntrSystem();
#endif
...
u32 Dp21RxSs_Pt_SetupIntrSystem(void)
{
	...
	XVFrmbufRd_SetCallback(&frmbufrd, XVFRMBUFRD_HANDLER_DONE,
			&Dp21RxSs_Pt_RbBufCallback, &frmbufrd);
	...
	XVFrmbufWr_SetCallback(&frmbufwr, XVFRMBUFWR_HANDLER_DONE,
			       &Dp21RxSs_Pt_WrBufCallback, &frmbufwr);
...
#endif
{
	...
	Dp21RxSs_Pt_PLLRefClkSel (&VPhyInst, XDP_TX_LINK_BW_SET_UHBR10, VPHY_TX_CHANNEL_TYPE);
	Dp21RxSs_Pt_PLLRefClkSel (&VPhyInst, XDP_TX_LINK_BW_SET_UHBR10, VPHY_RX_CHANNEL_TYPE);
		    ...
		    XVidC_VideoStream *StreamPtr)
{
	 ...
	 * Enable Interrupt
	 */
	XVFrmbufWr_InterruptEnable(&frmbufwr, XVFRMBUFWR_HANDLER_DONE);
...
#endif
#ifdef Rxo
	Dp21RxSs_Pt_FbStopWr();
#endif
}

#ifdef Txo
void Dp21RxSs_Pt_FbStopRd()
{
#if XPAR_XV_FRMBUFRD_NUM_INSTANCES
	u32 Status;
	fb_rd_start = 0;
...
void Dp21RxSs_Pt_FbStartWr(XDpTxSs_MainStreamAttributes Msa[4])
{
#if XPAR_XV_FRMBUFWR_NUM_INSTANCES
	...
	u32 stride = Dp21RxSs_Pt_CalcStride(Cfmt, AXIMMWIDTH, &VidStream);

	Dp21RxSs_Pt_ConfigWrFb(stride, Cfmt, &VidStream);
	stopped = 1;
	fb_wr_count = 0;
#endif
}

void Dp21RxSs_Pt_FbStartRd(XDpTxSs_MainStreamAttributes Msa[4])
{
#if XPAR_XV_FRMBUFRD_NUM_INSTANCES
	XVidC_ColorFormat Cfmt;
	XVidC_VideoStream VidStream;
	...
	if (Clip_4k == 1) {
		...
		VidStream.VmId = XVIDC_VM_3840x2160_30_P; /**< This will be set as 4K30 */
		TimingPtr = XVidC_GetTimingInfo(VidStream.VmId);
		VidStream.Timing = *TimingPtr;
		VidStream.FrameRate = XVidC_GetFrameRate(VidStream.VmId);
	}

	Dp21RxSs_Pt_ConfigRdFb(stride, Cfmt, &VidStream);
	fb_rd_start = 1;
	fb_rd_count = 0;
#endif
}
...
#if XPAR_XV_FRMBUFWR_NUM_INSTANCES
void Dp21RxSs_Pt_WrBufCallback(void)
{
	u32 Status;

	if (XVFRMBUFWR_BUFFER_BASEADDR >= (0 + (ADDR1) + (DELTA * 2))) {
		XVFRMBUFRD_BUFFER_BASEADDR = (0 + (ADDR1) + (DELTA * 1) + offset_rd);
		XVFRMBUFRD_BUFFER_BASEADDR_Y = (0 + (ADDR1_Y) + (DELTA * 1) + offset_rd);
...
#if XPAR_XV_FRMBUFRD_NUM_INSTANCES
void Dp21RxSs_Pt_RbBufCallback(void)
{
	stopped = 1;
...
int Dp21RxSs_Pt_DetectResolution(XDpRxSs *InstancePtr)
{
	char *color;

	Msa_Pt = XDpRxss_GetMsa(InstancePtr);
	Dp21RxSs_Pt_FbStopWr();
	XDp_RxSetLineReset(InstancePtr->DpPtr, XDP_TX_STREAM_ID1);
	XDp_RxDtgDis(InstancePtr->DpPtr);
	...
	if (InstancePtr->link_up_trigger == 1) {
		xil_printf("*** Resolution: %lu x %lu @ %luHz, BPC = %lu, Color = %s ***\r\n",
				Msa_Pt[0].Vtm.Timing.HActive,
				Msa_Pt[0].Vtm.Timing.VActive,
				Msa_Pt[0].Vtm.FrameRate,
				Msa_Pt[0].BitsPerColor,
				color);
	}

	if (InstancePtr->link_up_trigger == 1) {
#if XPAR_XV_FRMBUFWR_NUM_INSTANCES
		Dp21RxSs_Pt_FbStartWr(Msa_Pt);
#endif
		XDp_RxDtgEn(InstancePtr->DpPtr);
	}

	Dp21RxSs_Pt_CalculateCRC();
	return 1;
}
