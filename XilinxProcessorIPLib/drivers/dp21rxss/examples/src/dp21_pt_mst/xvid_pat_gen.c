...
#define XILINX_DISPLAYPORT_VID_BASE_ADDRESS		\
	XPAR_DP_TX_HIER_0_AV_PATGEN_0_BASEADDR
...
#if XPAR_DP_TX_HIER_0_V_DP_TXSS2_0_DP_OCTA_PIXEL_ENABLE
	...
	if (InstancePtr->TxInstance.MstEnable) {
		for (StreamIndex = 0; StreamIndex < 4; StreamIndex++) {
			if (XDp_TxMstStreamIsEnabled(InstancePtr,
				 ...
				 */
				XPatGen_WriteReg(XILINX_DISPLAYPORT_VID_BASE_ADDRESS,
...
void Vpg_Reset(void)
{
	...
	for (StreamIndex = 0; StreamIndex < 4; StreamIndex++) {
		XPatGen_WriteReg((XILINX_DISPLAYPORT_VID_BASE_ADDRESS) +
				 StreamOffset[StreamIndex], PatternGeneratorEnable,0x0);
	}
}
