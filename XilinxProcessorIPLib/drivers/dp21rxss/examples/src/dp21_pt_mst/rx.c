...
extern Video_CRC_Config VidFrameCRC_rx_4;	/**< Video Frame CRC instance */
...
u32 DpRxSs_Setup(void)
{
	 ...
	 * Changes for mst
	 */
	ReadVal = Dp21RxSs_PtMst_Read(DpRxSsInst.DpPtr->Config.BaseAddr, 0xD0);

	xil_printf("0XD0=%x\r\n", ReadVal);

	Dp21RxSs_PtMst_Write(DpRxSsInst.DpPtr->Config.BaseAddr, 0xD0, ReadVal);

	/*
	 * Disable Rx
	 */
	Dp21RxSs_PtMst_Write(DpRxSsInst.DpPtr->Config.BaseAddr, XDP_RX_LINK_ENABLE, 0x0);
	 ...
	 * working with R5.
	 */
	ReadVal = Dp21RxSs_PtMst_Read(DpRxSsInst.DpPtr->Config.BaseAddr,
	 ...
	 * Enable CRC Support
	 */
	Dp21RxSs_PtMst_Write(DpRxSsInst.DpPtr->Config.BaseAddr, XDP_RX_CRC_CONFIG,
	 ...
	 */
	Dp21RxSs_PtMst_Write(DpRxSsInst.DpPtr->Config.BaseAddr, XDP_RX_OVER_CTRL_DPCD, 0x1);
	Dp21RxSs_PtMst_Write(DpRxSsInst.DpPtr->Config.BaseAddr, 0x1600, 0x1);
	Dp21RxSs_PtMst_Write(DpRxSsInst.DpPtr->Config.BaseAddr, XDP_RX_OVER_CTRL_DPCD, 0x0);

	/*
	 * Enable Rx
	 */
	Dp21RxSs_PtMst_Write(DpRxSsInst.DpPtr->Config.BaseAddr, XDP_RX_LINK_ENABLE, 0x1);
...
void Set_EDID(u8 edid_monitor[384])
{
	...
	for (i = 0; i < (384*4); i = i+(16*4)) {
		for (j = i; j < (i+(16*4)); j = j+4) {
			Dp21RxSs_PtMst_Write (VID_EDID_BASEADDR, j, edid_monitor[(i/4)+1]);
		}
	}

	for (i = 0; i < (384*4); i = i+4) {
		Dp21RxSs_PtMst_Write (VID_EDID_BASEADDR, i, edid_monitor[i/4]);
	}
}
/*****************************************************************************/
/**
...
void Dprx_InterruptHandlerPayloadAlloc(void *InstancePtr)
{
	...
	RegVal = Dp21RxSs_PtMst_Read(DpPtr->Config.BaseAddr, XDP_RX_MST_ALLOC);
...
void Dprx_InterruptHandlerActRx(void *InstancePtr)
{
	...
	XDp_RxInterruptEnable1(DpRxSsInst.DpPtr, 0x3FFFF);
}
...
void DpRxSs_TrainingDoneHandler(void *InstancePtr)
{
	...
	ReadVal = Dp21RxSs_PtMst_Read(DpRxSsInst.DpPtr->Config.BaseAddr, XDP_RX_AUX_CLK_DIVIDER);
	ReadVal = ReadVal & 0xF0FF00FF;
	ReadVal = ReadVal | (0<<24);

	Dp21RxSs_PtMst_Write(DpRxSsInst.DpPtr->Config.BaseAddr, XDP_RX_AUX_CLK_DIVIDER,
		      ReadVal);
	...
	XDp_RxInterruptEnable(DpRxSsInst.DpPtr, XDP_RX_INTERRUPT_MASK_VIDEO_MASK);
}
...
void DpRxSs_LinkBandwidthHandler(void *InstancePtr)
{
	...
	Status = Dp21RxSs_PtMst_PhyConfig(&VPhyInst, DpRxSsInst.UsrOpt.LinkRate, VPHY_RX_PLL_TYPE,
...
void DpRxSs_CRCTestEventHandler(void *InstancePtr)
{
	...
	ReadVal = Dp21RxSs_PtMst_Read(DpRxSsInst.DpPtr->Config.BaseAddr, XDP_RX_CRC_CONFIG);

	/*
	 * Record Training Algo Value - to be restored in non-phy test mode
	 */
	TrainingAlgoValue = Dp21RxSs_PtMst_Read(DpRxSsInst.Config.BaseAddress,
					 XDP_RX_MIN_VOLTAGE_SWING);
	...
	if ((ReadVal&0x8000) == 0x8000) {
		 ...
		 */
		ReadVal = Dp21RxSs_PtMst_Read(DpRxSsInst.Config.BaseAddress,
				       XDP_RX_CDR_CONTROL_CONFIG);
		Dp21RxSs_PtMst_Write(DpRxSsInst.DpPtr->Config.BaseAddr,
...
void DpRxSs_AccessLinkQualHandler(void *InstancePtr)
{
	...
	LinkQualConfig = Dp21RxSs_PtMst_Read(DpRxSsInst.DpPtr->Config.BaseAddr,
				      XDP_RX_DPC_LINK_QUAL_CONFIG);

	/*
	 * AUX Defer is needed mainly for PRBS error counter transfer
	 *
	 * Paranoid defer for error counter reads to ensure value is properly
	 * reported
	 */
	...
	if((LinkQualConfig & 0x00000007) == XDP_RX_DPCD_LINK_QUAL_PRBS) {
		...
		Dp21RxSs_PtMst_Write(VPhyInst.Config.BaseAddr, XVPHY_RX_CONTROL_REG, DrpVal);
...
void DpRxSs_AccessErrorCounterHandler(void *InstancePtr)
{
	 ...
	 */
	SymErrVal_lane01 = XVphy_ReadReg(VIDPHY_BASEADDR,
					 XVPHY_RX_SYM_ERR_CNTR_CH1_2_REG);
	SymErrVal_lane23 = XVphy_ReadReg(VIDPHY_BASEADDR,
	...
	XVphy_DrpRd(&VPhyInst, 0, XVPHY_CHANNEL_ID_CH2,
		    XVPHY_DRP_GTHE4_PRBS_ERR_CNTR_LOWER,
		    &tmp);
	DrpVal_lower_lane1 = tmp;
	XVphy_DrpRd(&VPhyInst, 0, XVPHY_CHANNEL_ID_CH2,
		    XVPHY_DRP_GTHE4_PRBS_ERR_CNTR_UPPER, &tmp);
	DrpVal = tmp;
	...
	if(((ReadVal&0x7) == XDP_RX_DPCD_LINK_QUAL_PRBS) &&
		...
		Dp21RxSs_PtMst_Write(VPhyInst.Config.BaseAddr, XVPHY_RX_CONTROL_REG, DrpValConfig);

		DrpValConfig = XVphy_ReadReg(VPhyInst.Config.BaseAddr, XVPHY_RX_CONTROL_REG);
		DrpValConfig = DrpValConfig & 0xF7F7F7F7;
		XVphy_WriteReg(VPhyInst.Config.BaseAddr, XVPHY_RX_CONTROL_REG, DrpValConfig);
	}
#endif
}
...
void ReportVideoCRC(u8 select)
{
	if (!select) {
		...
		xil_printf("-----Stream 2-------\r\n");
		XVidFrameCrc_Report(&VidFrameCRC_rx_1);

		xil_printf("-----Stream 3-------\r\n");
		XVidFrameCrc_Report(&VidFrameCRC_rx_2);

		xil_printf("-----Stream 4-------\r\n");
		XVidFrameCrc_Report(&VidFrameCRC_rx_3);
	} else {
		if (select == 1) {
			xil_printf("-----Stream 1-------\r\n");
			XVidFrameCrc_Report(&VidFrameCRC_rx_4);
