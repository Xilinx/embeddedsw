...
// Write SAMPRATE field
 data|=((SampleRate & AUD_CFG_REG_SAMPRATE_MASK) << AUD_CFG_REG_SAMPRATE_SHIFT);
...
// Write PATTERN field
    data |=((Pattern & CH_CTRL_REG_PATTERN_MASK) << CH_CTRL_REG_PATTERN_SHIFT);
// Write channel reg
...
// Write NUMCHANS field
    data |= ((NumChannels & AUD_CFG_REG_NUMCH_MASK) << AUD_CFG_REG_NUMCH_SHIFT);
// Write audio config reg
    XAudGen_WriteReg(AudioGen->AudGenBase, AUD_CFG, data);
