...
#define GPIO_SENSOR		XPAR_MIPI_CSI_RX_SS_AXIGPIOSEN_BASEADDR

#define GPIO_IP_RESET	 XPAR_GPIO_1_BASEADDR
#else
#define UART_BASEADDR		XPAR_SERIAL0_BASEADDR
#define MIPICSI_BASEADDR	XPAR_XMIPICSISS_0_BASEADDR
#define GPIO_SENSOR		XPAR_XGPIO_2_BASEADDR
#define GPIO_IP_RESET	 	XPAR_XGPIO_0_BASEADDR
#endif
...
#define DDR_BASEADDR XPAR_AXI_NOC2_DDR_LOW_0_BASEADDR
...
void Reset_IP_Pipe(void)
{
	...
	Xil_Out32(GPIO_IP_RESET, 0xFFFFFFFF);
#else
	Xil_Out32(XPAR_XGPIO_0_BASEADDR, 0xFFFFFFFF);
	Xil_Out32(XPAR_XGPIO_0_BASEADDR, 0x00000000);
	Xil_Out32(XPAR_XGPIO_0_BASEADDR, 0xFFFFFFFF);
#endif
}
...
/*****************************************************************************/
/**
* This function resets OS05A10 camera sensor.
*
* @return	None.
*
* @note		None.
...
void CamReset(void)
{
	...
	Xil_Out32(GPIO_SENSOR, 0x07);
#else
	Xil_Out32(XPAR_XGPIO_2_BASEADDR, 0x1);
	Xil_Out32(XPAR_XGPIO_2_BASEADDR, 0x0);
	Xil_Out32(XPAR_XGPIO_2_BASEADDR, 0x1);
#endif
}
...
int SetupCameraSensor(void) {
	...
	switch (Pipeline_Cfg.VideoMode) {
		...
		case XVIDC_VM_1280x720_60_P:
			MaxIndex = length_OS05A10_config_1080p_60fps_regs;
			sensor_cfg = OS05A10_config_1080p_60fps_regs;
			break;
...
WriteBuffer[0] = 0x300a >>8;
WriteBuffer[1] = 0x300a;
ReadBuffer[0] = 0;
Status = SensorReadData(ReadBuffer, 1);
usleep(100);
WriteBuffer[0] = 0x300b >>8;
WriteBuffer[1] = 0x300b;
ReadBuffer[0] = 0;
Status = SensorReadData(ReadBuffer, 1);
usleep(100);
WriteBuffer[0] = 0x300c >>8;
WriteBuffer[1] = 0x300c;
ReadBuffer[0] = 0;
Status = SensorReadData(ReadBuffer, 1);
usleep(100);
WriteBuffer[0] = 0x0103 >>8;
WriteBuffer[1] = 0x0103;
WriteBuffer[2] = 0x01;
...
#else
	CsiRxSsCfgPtr = XCsiSs_LookupConfig(XPAR_XMIPICSISS_0_BASEADDR);
#endif
