...
)
{
	...
	result = Send_Command(APU_2_RPU_MB_CMD_LSC_SET_CONFIG, &packet,
...
)
{
	...
	result = Send_Command(APU_2_RPU_MB_CMD_LSC_GET_CONFIG, &packet,
			      packet.payload_size + payload_extra_size, dest_cpu_id, src_cpu_id);
	if (0 != result)
		return RET_FAILURE;
...
)
{
	...
	result = Send_Command(APU_2_RPU_MB_CMD_LSC_ENABLE, &packet,
			      packet.payload_size + payload_extra_size, dest_cpu_id, src_cpu_id);
	if (0 != result)
		return RET_FAILURE;
	packet.resp_field.error_subcode_t = apu_wait_for_ACK(packet.cookie,
...
)
{
	...
	result = Send_Command(APU_2_RPU_MB_CMD_LSC_DISABLE, &packet,
			      packet.payload_size + payload_extra_size, dest_cpu_id, src_cpu_id);
	if (0 != result)
		return RET_FAILURE;
	packet.resp_field.error_subcode_t = apu_wait_for_ACK(packet.cookie,
...
)
{
	...
	result = Send_Command(APU_2_RPU_MB_CMD_LSC_RESET, &packet, packet.payload_size + payload_extra_size,
...
)
{
	...
	result = Send_Command(APU_2_RPU_MB_CMD_LSC_GET_STATUS, &packet,
			      packet.payload_size + payload_extra_size, dest_cpu_id, src_cpu_id);
	if (0 != result)
		return RET_FAILURE;
	packet.resp_field.error_subcode_t = apu_wait_for_ACK(packet.cookie, packet.payload_data);
...
)
{
	...
	result = Send_Command(APU_2_RPU_MB_CMD_LSC_GET_VERSION, &packet,
			      packet.payload_size + payload_extra_size, dest_cpu_id, src_cpu_id);
	if (0 != result)
		return RET_FAILURE;
	packet.resp_field.error_subcode_t = apu_wait_for_ACK(packet.cookie, packet.payload_data);
