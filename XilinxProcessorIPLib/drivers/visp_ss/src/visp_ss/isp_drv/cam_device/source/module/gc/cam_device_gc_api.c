...
)
{
	...
	result = Send_Command(APU_2_RPU_MB_CMD_GC_ENABLE, &packet, packet.payload_size + payload_extra_size,
...
)
{
	...
	result = Send_Command(APU_2_RPU_MB_CMD_GC_RESET, &packet, packet.payload_size + payload_extra_size,
			      dest_cpu_id, src_cpu_id);
	if (0 != result)
		return RET_FAILURE;
	packet.resp_field.error_subcode_t = apu_wait_for_ACK(packet.cookie,
