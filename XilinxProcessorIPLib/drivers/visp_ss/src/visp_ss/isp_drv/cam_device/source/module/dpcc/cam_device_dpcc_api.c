...
)
{
	...
	memcpy(p_data, pDpccCfg, sizeof(CamDeviceDpccConfig_t));
	packet.payload_size += sizeof(CamDeviceDpccConfig_t);

	if (packet.payload_size > MAX_ITEM)
		return RET_OUTOFRANGE;

	result = Send_Command(APU_2_RPU_MB_CMD_DPCC_SET_CONFIG, &packet,
...
)
{
	...
	result = Send_Command(APU_2_RPU_MB_CMD_DPCC_GET_CONFIG, &packet,
			      packet.payload_size + payload_extra_size, dest_cpu_id, src_cpu_id);
	if (0 != result)
		return RET_FAILURE;
	packet.resp_field.error_subcode_t = apu_wait_for_ACK(packet.cookie, packet.payload_data);
	memcpy(pDpccCfg, p_data, sizeof(CamDeviceDpccConfig_t));
...
)
{
	...
	result = Send_Command(APU_2_RPU_MB_CMD_DPCC_ENABLE, &packet,
			      packet.payload_size + payload_extra_size, dest_cpu_id, src_cpu_id);
	if (0 != result)
		return RET_FAILURE;
	packet.resp_field.error_subcode_t = apu_wait_for_ACK(packet.cookie,
...
)
{
	...
	result = Send_Command(APU_2_RPU_MB_CMD_DPCC_DISABLE, &packet,
			      packet.payload_size + payload_extra_size, dest_cpu_id, src_cpu_id);
	if (0 != result)
		return RET_FAILURE;
	packet.resp_field.error_subcode_t = apu_wait_for_ACK(packet.cookie,
...
)
{
	...
	p_data += sizeof(uint32_t);
	packet.payload_size += sizeof(uint32_t);
	memcpy(p_data, pDpccStatus, sizeof(CamDeviceDpccStatus_t));
	packet.payload_size += sizeof(CamDeviceDpccStatus_t);

	if (packet.payload_size > MAX_ITEM)
		return RET_OUTOFRANGE;

	result = Send_Command(APU_2_RPU_MB_CMD_DPCC_GET_STATUS, &packet,
			      packet.payload_size + payload_extra_size, dest_cpu_id, src_cpu_id);
	if (0 != result)
		return RET_FAILURE;
	packet.resp_field.error_subcode_t = apu_wait_for_ACK(packet.cookie, packet.payload_data);
	memcpy(pDpccStatus, p_data, sizeof(CamDeviceDpccStatus_t));
...
)
{
	...
	result = Send_Command(APU_2_RPU_MB_CMD_DPCC_RESET, &packet,
			      packet.payload_size + payload_extra_size, dest_cpu_id, src_cpu_id);
	if (0 != result)
		return RET_FAILURE;
	packet.resp_field.error_subcode_t = apu_wait_for_ACK(packet.cookie,
...
)
{
	...
	result = Send_Command(APU_2_RPU_MB_CMD_DPCC_GET_VERSION, &packet,
			      packet.payload_size + payload_extra_size, dest_cpu_id, src_cpu_id);
	if (0 != result)
		return RET_FAILURE;
	packet.resp_field.error_subcode_t = apu_wait_for_ACK(packet.cookie, packet.payload_data);
