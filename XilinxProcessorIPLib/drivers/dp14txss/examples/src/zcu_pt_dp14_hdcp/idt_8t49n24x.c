				...
				IDT_8T49N24x_Settings* RegSettings)
{
	...
	/* Only use the divide-by-1 option for really small divide
	 * ratios note that this option will never be on the list
	 * for the Q0 - Q3 dividers. */
	if (MaxDiv < 4) {

	}

	/* Make sure we can divide the ratio by 4 in NS1 and by 1
...
//	int M1Min = (int)(FVCO/IDT_8T49N24X_FPD_MAX);
	...
	/* Start from lowest divider and iterate until 0
	 * error is found or the divider limit is exhausted.
	 * */
...
/*
	if (!FreeRun) {
		...
		// Configure device
//		Result = IDT_8T49N24x_Configure(I2CBaseAddress,
//						I2CSlaveAddress);
*/
	// Mode
	if (FreeRun == TRUE) {
			...
			int FIn, int FOut, u8 FreeRun)
{
	...
	Buffer[1] = 0x0A; // LSB
	if (FreeRun == TRUE) {
		// STATE = Force FREERUN and disable input references
		Buffer[2] = 0x31;
	} else {
		// STATE = Force NORMAL, rest is default
		Buffer[2] = 0x02;
	}
	ByteCount = XIic_Send(I2CBaseAddress, I2CSlaveAddress,
			      (u8*)Buffer, 3, XIIC_STOP);
...
int IDT_8T49N24x_Init(u32 I2CBaseAddress, u8 I2CSlaveAddress)
{
	...
	/* Check device ID */
//	Result = IDT_8T49N24x_CheckDeviceID(I2CBaseAddress,
//					    I2CSlaveAddress);
	...
	if (Result == XST_SUCCESS) {
		 ...
		 * During the PLL parameters update,
		 * the i2c might become unresponsive.
...
static int IDT_8T49N24x_Configure_JA(u32 I2CBaseAddress, u8 I2CSlaveAddress)
{
	...
	/* The configuration is started from address 0x08. */
	for (Index = 8 ;
	     Index < sizeof(IDT_8T49N24x_Config_JA) ;
	     Index++) {
		/* Skip address 0x70. */
		/* Address 0x70 enables the DPLL and APLL calibration. */
		if (Index != 0x070) {
...
int IDT_8T49N24x_GetIntDivTable(int FOut, int *DivTable, u8 Bypass)
{
	...
	if (NS2Min == 1) {
		...
		if (NS2Max == 0) {
			/* Because we're rounding-down for the
			 * max, we may end-up with it being 0,
			 * in which case we need to make it 1.
			 * */
			NS2Max = 1;
		}
			...
			IDT_8T49N24x_Settings* RegSettings)
{
	...
	/* Only use the divide-by-1 option
	 * for really small divide ratios
	 * Note that this option will never be
	...
	/* Make sure we can divide the ratio by 4 in
	 * NS1 and by 1 or an even number in NS2.
	 * */
	if ((MaxDiv == 4) ||
	    (MaxDiv % 8 == 0)) {
	...
	/* Make sure we can divide the ratio by 5 in
	 * NS1 and by 1 or an even number in NS2
	 * */
	if ((MaxDiv == 5) ||
	    (MaxDiv % 10 == 0)) {
	...
	/* Make sure we can divide the ratio by 6 in NS1
...
//	int M1Min = (int)(FVCO/IDT_8T49N24X_FPD_MAX);
	...
	/* Start from lowest divider and iterate until 0 error is
	 * found or the divider limit is exhausted.
	 * Keep the setting with the lowest error
	 * */
	for (i = PMin; i <= IDT_8T49N24X_P_MAX; i++) {
