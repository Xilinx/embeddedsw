...
void CreateDescriptors(RunConfig *RunCfgPtr, XDcDma_Descriptor *XDesc, FrameInfo *FBInfo, XDcDma_Descriptor *NextDesc)
{
	...
	if (RunCfgPtr->SdpEnable) {
		xil_printf("sdp enabled for desc here!\n");
		XDesc->Target_Addr = 0;
	} else if (RunCfgPtr->CursorEnable){
		xil_printf("Cursor enabled for desc here!\n");
		XDesc->Target_Addr = 1;
	} else {
...
void GenerateSdpPacket(RunConfig *RunCfgPtr)
{
	...
	u8 Channel = 0x8;
	u8 CA = 0x13;
	u8 SampleSize = 0x1; /* AudWidth = 16bit */
	u8 AudFreqSel = 0x3; /* 48KkHz */

	BufAddr = SDP_BUFFER_ADDR;

	for(Index = 0; Index < 64; Index++)
	{
		xil_printf("\n SDP packet %d Data Loading Start", Index);

		XDc_WriteReg(BufAddr , 0x0, 0x4B1B8400);
		XDc_WriteReg(BufAddr , 0x4, (0x00000010 | CA <<24 | AudFreqSel << 10 | SampleSize << 8 | (Channel - 1)));
		...
		BufAddr += XDC_SDP_BUFFER_OFFSET;

		xil_printf("\n SDP packet %d Data Loading End", Index);
	}
}
...
void InitFrames(RunConfig *RunCfgPtr)
{
	...
	if(RunCfgPtr->SdpEnable)
	{
		...
		Sdp_FbInfo->Address = SDP_BUFFER_ADDR;
		Sdp_FbInfo->Width = 128;
		Sdp_FbInfo->Height = 128;
		Sdp_FbInfo->VideoFormat = RGBA4444;
		Sdp_FbInfo->Bpc = 2;
		GenerateFrameInfoAttribute(Sdp_FbInfo);

		CreateDescriptors(RunCfgPtr, RunCfgPtr->Desc7, RunCfgPtr->Sdp_FbInfo, NULL);

		GenerateSdpPacket(RunCfgPtr);
	}
...
u32 InitRunConfig(RunConfig *RunCfgPtr)
{
	...
	RunCfgPtr->AudioEnable = 0;

	/* Enable/Disable Sdp */
	RunCfgPtr->SdpEnable = 1;
