      ...
      UINTPTR EffectiveAddr)
{
	...
	InstancePtr->Handles.RxDpAuxReadCallback = XHdcp22_Rx_StubRdWrHandler;
	InstancePtr->Handles.IsRxDpAuxReadCallbackSet = FALSE;

	InstancePtr->Handles.RxDpAuxWriteCallback = XHdcp22_Rx_StubRdWrHandler;
	InstancePtr->Handles.IsRxDpAuxWriteCallbackSet = FALSE;

	InstancePtr->Handles.RxDpCpIrqSetCallback = XHdcp22_Rx_StubGetHandler;
	InstancePtr->Handles.IsRxDpCpIrqSetCallbackSet = FALSE;
	...
	InstancePtr->RxCaps[2] =
		(InstancePtr->Config.Mode == XHDCP22_RX_RECEIVER) ?
		XHDCP22_RXCAPS_HDCP_ENABLE : XHDCP22_RXCAPS_REPEATER;
	...
	if(Status == XST_FAILURE)
	{
		xdbg_printf(XDBG_DEBUG_GENERAL,
			"HDCP22RX ERR:: Cipher Initialization failed \n\r");
		return Status;
	}
	...
	{
		xdbg_printf(XDBG_DEBUG_GENERAL,
			"HDCP22RX ERR:: Mmult Initialization failed \n\r");
		return Status;
	}
	...
	{
		xdbg_printf(XDBG_DEBUG_GENERAL,
			"HDCP22RX ERR:: Rng Initialization failed \n\r");
		return Status;
	}
...
int XHdcp22Rx_Dp_Reset(XHdcp22_Rx_Dp *InstancePtr)
{
	...
	if (InstancePtr->Config.Protocol != XHDCP22_RX_DP) {
		...
		Xil_AssertNonvoid(InstancePtr->Handles.IsDdcAllCallbacksSet);
	}
...
int XHdcp22Rx_Dp_SetCallback(XHdcp22_Rx_Dp *InstancePtr, XHdcp22_Rx_Dp_HandlerType HandlerType, void *CallbackFunc, void *CallbackRef)
{
	...
	switch(HandlerType)
	{
		...
		case (XHDCP22_RX_HANDLER_DP_AUX_READ):
			InstancePtr->Handles.RxDpAuxReadCallback =
				(Xhdcp22_Rx_RdWrHandler)CallbackFunc;
			InstancePtr->Handles.RxDpAuxReadCallbackRef =
				CallbackRef;
			InstancePtr->Handles.IsRxDpAuxReadCallbackSet = TRUE;
			Status = XST_SUCCESS;
			break;

		case (XHDCP22_RX_HANDLER_DP_AUX_WRITE):
			InstancePtr->Handles.RxDpAuxWriteCallback =
				(Xhdcp22_Rx_RdWrHandler)CallbackFunc;
			InstancePtr->Handles.RxDpAuxWriteCallbackRef =
				CallbackRef;
			InstancePtr->Handles.IsRxDpAuxWriteCallbackSet =
				TRUE;
			Status = XST_SUCCESS;
			break;

		case (XHDCP22_RX_HANDLER_DP_CP_IRQ_SET):
			InstancePtr->Handles.RxDpCpIrqSetCallback =
				(XHdcp22_Rx_GetHandler)CallbackFunc;
			InstancePtr->Handles.RxDpCpIrqSetCallbackRef =
				CallbackRef;
			InstancePtr->Handles.IsRxDpCpIrqSetCallbackSet =
				TRUE;
			Status = XST_SUCCESS;
			break;

		case (XHDCP22_RX_HANDLER_DP_AUX_DEFER_SET_CLR):
			InstancePtr->Handles.RxDpSetClearAuxDeferCallback =
				(XHdcp22_Rx_SetHandler)CallbackFunc;
			InstancePtr->Handles.RxDpSetClearAuxDeferCallbackRef =
				CallbackRef;
			InstancePtr->Handles.
				IsRxDpSetClearAuxDeferCallbackSet = TRUE;
			Status = XST_SUCCESS;
			break;
...
static u8 XHdcp22Rx_IsWriteMessageAvailable(XHdcp22_Rx_Dp *InstancePtr)
{
	...
	/* Check if any DPCD Msg is available*/
	if(InstancePtr->Info.DpcdFlag != XHDCP22_RX_DPCD_FLAG_NONE)
		return TRUE;
...
static void XHdcp22Rx_SetDdcReauthReq(XHdcp22_Rx_Dp *InstancePtr)
{
	...
	if (InstancePtr->Config.Protocol == XHDCP22_RX_DP) {
		...
		XHdcp22Rx_SetRxStatus(InstancePtr, 0,
				InstancePtr->Info.ReauthReq,
				InstancePtr->Info.TopologyReady);
	}
...
static int XHdcp22Rx_PollMessage(XHdcp22_Rx_Dp *InstancePtr)
{
	...
	if(XHdcp22Rx_IsWriteMessageAvailable(InstancePtr) == TRUE)
	{
		if (InstancePtr->Config.Protocol == XHDCP22_RX_HDMI) {
			/*Read received msg through DDC callbacks*/
			...
			/*Read received msg through DPCD callbacks*/
			Size = XHdcp22Rx_Dp_Read_Dpcd_Msg(InstancePtr);
		}
	}
...
static int XHdcp22Rx_ProcessMessageAKEInit(XHdcp22_Rx_Dp *InstancePtr)
{
	...
	if (InstancePtr->Config.Protocol == XHDCP22_RX_DP) {
		...
		InstancePtr->Handles.RxDpSetClearAuxDeferCallback(
				InstancePtr->Handles.
					RxDpSetClearAuxDeferCallbackRef,
				TRUE);
	}
...
static int XHdcp22Rx_SendMessageAKESendHPrime(XHdcp22_Rx_Dp *InstancePtr)
{
	...
	if (InstancePtr->Config.Protocol == XHDCP22_RX_DP) {
		...
		/*Write H'_AVAILABLE in RxStatus*/
		InstancePtr->Handles.RxDpAuxWriteCallback(
				InstancePtr->Handles.RxDpAuxWriteCallbackRef,
				RX_STATUS_OFFSET, &rxstatus, 1);

		/*Clear AUX_DEFERS for H' message*/
		InstancePtr->Handles.RxDpSetClearAuxDeferCallback(
				InstancePtr->Handles.RxDpSetClearAuxDeferCallbackRef,
				(FALSE));
...
static int XHdcp22Rx_SendMessageAKESendPairingInfo(XHdcp22_Rx_Dp *InstancePtr)
{
	...
	if (InstancePtr->Config.Protocol == XHDCP22_RX_DP) {
		/*Write Pairing_Info_AVAILABLE in RxStatus*/
		u8 rxstatus = RX_STATUS_PAIRING_AVAILABLE;
		InstancePtr->Handles.RxDpAuxWriteCallback(
				InstancePtr->Handles.RxDpAuxWriteCallbackRef,
				RX_STATUS_OFFSET, &rxstatus, 1);

		/*Clear AUX_DEFERS for Pairing_Info message*/
		InstancePtr->Handles.RxDpSetClearAuxDeferCallback(
				InstancePtr->Handles.
					RxDpSetClearAuxDeferCallbackRef,
				FALSE);
...
static int XHdcp22Rx_SendMessageLCSendLPrime(XHdcp22_Rx_Dp *InstancePtr)
{
	...
	/* From the spec there is nothing to notify to
	 * the Upstream TX that L' is available,
	 * So nothing to be done for DP*/
	if (InstancePtr->Config.Protocol != XHDCP22_RX_DP)
	{
		/* Write message size signaling completion */
 ...
 *		layer when setting the callback functions, and passed back to
 *		the upper layer when the callback is invoked.
 * @param	offset is a offset to be read/write from/to.
 * @param 	buf is a pointer to the dat to be written/read into/from
 * @param	size is the size of data to be written/read
 *
 * @return	size of data that is written/read.
 *
 * @note		None.
 ******************************************************************************/
static u32 XHdcp22_Rx_StubRdWrHandler(void *HandlerRef, u32 offset,
		u8 *buf, u32 size)
{
	Xil_AssertNonvoid(HandlerRef);
	Xil_AssertNonvoid(offset);
	Xil_AssertNonvoid(buf);
	Xil_AssertNonvoid(size);
	Xil_AssertNonvoidAlways();
	...
	switch(dpcdflag) {
		case XHDCP22_RX_DPCD_FLAG_AKE_INIT_RCVD:
			...
			size = InstancePtr->Handles.RxDpAuxReadCallback(
					...
					R_TX_OFFSET, buf,
					R_TX_SIZE + TX_CAPS_SIZE);
			/*Put this msg into MsgBuffer*/
			InstancePtr->MessageBuffer[0] =
				XHDCP22_RX_MSG_ID_AKEINIT; /*Refer Note*/
		...
		case XHDCP22_RX_DPCD_FLAG_AKE_NO_STORED_KM_RCVD:
			...
			size = InstancePtr->Handles.RxDpAuxReadCallback(
					InstancePtr->Handles.
						RxDpAuxReadCallbackRef,
					E_KPUB_KM_OFFSET, buf,
					E_KPUB_KM_SIZE);
			/*Put this msg into MsgBuffer*/
			InstancePtr->MessageBuffer[0] =
				XHDCP22_RX_MSG_ID_AKENOSTOREDKM; /*Refer Note*/
			memcpy(&InstancePtr->MessageBuffer[1], buf,
					E_KPUB_KM_SIZE);
		...
		case XHDCP22_RX_DPCD_FLAG_AKE_STORED_KM_RCVD:
			...
			/*Put this msg into MsgBuffer*/
			InstancePtr->MessageBuffer[0] =
				XHDCP22_RX_MSG_ID_AKESTOREDKM; /*Refer Note*/
			memcpy(&InstancePtr->MessageBuffer[1], buf,
					E_KH_KM_SIZE + M_SIZE);
			/*This is for Message Id InstancePtr->MessageBuffer[0]*/
			size += 1;
		...
		case XHDCP22_RX_DPCD_FLAG_LC_INIT_RCVD:
			...
			/*Put this msg into MsgBuffer*/
			InstancePtr->MessageBuffer[0] =
				XHDCP22_RX_MSG_ID_LCINIT; /*Refer Note*/
			memcpy(&InstancePtr->MessageBuffer[1], buf,
					R_N_SIZE);
			/*This is for Message Id InstancePtr->MessageBuffer[0]*/
			size += 1;
		...
		case XHDCP22_RX_DPCD_FLAG_SKE_SEND_EKS_RCVD:
			...
			size = InstancePtr->Handles.RxDpAuxReadCallback(
					InstancePtr->Handles.
						RxDpAuxReadCallbackRef,
					E_DKEY_KS_OFFSET, buf,
					E_DKEY_KS_SIZE + R_IV_SIZE);
			/*Put this msg into MsgBuffer*/
			InstancePtr->MessageBuffer[0] =
				XHDCP22_RX_MSG_ID_SKESENDEKS; /*Refer Note*/
			memcpy(&InstancePtr->MessageBuffer[1], buf,
					E_DKEY_KS_SIZE + R_IV_SIZE);
			/*This is for Message Id InstancePtr->MessageBuffer[0]*/
			size += 1;
...
static int XHdcp22Rx_Dp_Write_Dpcd_Msg(XHdcp22_Rx_Dp *InstancePtr)
{
	...
	memcpy(&buffer, InstancePtr->MessageBuffer, sizeof(XHdcp22_Rx_Message));
	...
	switch (buffer.MsgId) {
		case XHDCP22_RX_MSG_ID_AKESENDCERT:
			...
			numwritten = InstancePtr->Handles.RxDpAuxWriteCallback(
					...
					CERT_RX_OFFSET,
					buffer.AKESendCert.CertRx,
					CERT_RX_SIZE + R_RX_SIZE + RX_CAPS_SIZE);
			break;
		case XHDCP22_RX_MSG_ID_AKESENDHPRIME:
			/*Write H'*/
			numwritten = InstancePtr->Handles.RxDpAuxWriteCallback(
					InstancePtr->Handles.
						RxDpAuxWriteCallbackRef,
					H_PRIME_OFFSET,
					buffer.AKESendHPrime.HPrime,
					H_PRIME_SIZE);
			break;
		case XHDCP22_RX_MSG_ID_AKESENDPAIRINGINFO:
			/*Write Ake_Send_Pairing info msg*/
			numwritten = InstancePtr->Handles.RxDpAuxWriteCallback(
					InstancePtr->Handles.
					RxDpAuxWriteCallbackRef,
...
void XHdcp22_Dp_timer_attach(XHdcp22_Rx_Dp *InstancePtr, XTmrCtr *TmrCtrPtr)
{
	...
	InstancePtr->TimerInst = *TmrCtrPtr;
