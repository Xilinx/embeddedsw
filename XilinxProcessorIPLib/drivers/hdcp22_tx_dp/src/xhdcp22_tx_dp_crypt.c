...
* @param InstancePtr is a pointer to the XHdcp22_Tx_Dp instance to be worked on.
...
/**
 *
 * This function computes the HDCP 2.2 verification value V using the
 * provided transmitter, receiver, and session parameters.
 *
 * @param Rn           Pointer to the transmitter nonce.
 * @param Rrx          Pointer to the receiver nonce.
 * @param RxInfo       Pointer to receiver information data.
 * @param Rtx          Pointer to the transmitter random value.
 * @param RecvIDList   Pointer to the list of receiver IDs.
 * @param RecvIDCount  Number of receiver IDs in RecvIDList.
 * @param SeqNum_V     Pointer to the sequence number used for V computation.
 * @param Km           Pointer to the session key.
 * @param V            Pointer to the output buffer where computed V is stored.
 *
 ******************************************************************************/
...
/**
 * Computes the authentication value M used in the HDCP 2.2 protocol.
 *
 * This function calculates the M value by combining various authentication
 * parameters according to the HDCP 2.2 specification for DisplayPort transmitters.
 *
 * @param Rn Pointer to the random number Rn.
 * @param Rrx Pointer to the receiver's random number Rrx.
 * @param Rtx Pointer to the transmitter's random number Rtx.
 * @param StreamIDType Pointer to the stream ID type value.
 * @param k Pointer to the k parameter value.
 * @param SeqNum_M Pointer to the sequence number for M computation.
 * @param Km Pointer to the master key Km.
 * @param M Pointer to the output buffer where the computed authentication
 *           value M will be stored.
 *
 * @note The caller must ensure all input pointers are valid and point to
 *       properly sized buffers. The V output buffer must be pre-allocated
 *       with sufficient size to store the computed hash.
 */
