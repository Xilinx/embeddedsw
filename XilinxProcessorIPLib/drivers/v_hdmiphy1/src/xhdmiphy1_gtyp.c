...
#if (XPAR_HDMIPHY1_0_TRANSCEIVER == XHDMIPHY1_GTYP)
#include "xstatus.h"

/**************************** Function Prototypes *****************************/

u32 XHdmiphy1_GtypRxChReconfig(XHdmiphy1 *InstancePtr, u8 QuadId,
        XHdmiphy1_ChannelId ChId);
u32 XHdmiphy1_GtypTxChReconfig(XHdmiphy1 *InstancePtr, u8 QuadId,
        XHdmiphy1_ChannelId ChId);
	...
	if (Dir == XHDMIPHY1_DIR_RX) {
		...
		if ((!IsHdmi21) && (!TmdsClockRatio) &&
			...
			if (InstancePtr->Config.DruIsPresent) {
				...
				LineRate = XHDMIPHY1_HDMI_GTYP_DRU_LRATE;
			}
			else {
