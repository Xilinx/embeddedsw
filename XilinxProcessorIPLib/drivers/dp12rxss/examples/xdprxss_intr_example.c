...
/************************** Function Prototypes ******************************/
#ifdef SDT
u32 DpRxSs_IntrExample(UINTPTR BaseAddress);
#else
...
#else
	Status = DpRxSs_IntrExample(XPAR_DPRXSS_0_BASEADDR);
#endif
...
u32 DpRxSs_SetupIntrSystem(void)
{
...
#if (XPAR_XHDCP_NUM_INSTANCES > 0)
	Status = XSetupInterruptSystem(&DpRxSsInst.Hdcp1xPtr, XDpRxSs_HdcpIntrHandler,
	...
	Status = XSetupInterruptSystem(&DpRxSsInst, XDpRxSs_TmrCtrIntrHandler,
				       ...
				       XINTERRUPT_DEFAULT_PRIORITY);
	if (Status != XST_SUCCESS) {
		xil_printf("ERR: RX Timer interrupt connect failed!\n\r");
		return XST_FAILURE;
	}
#endif
