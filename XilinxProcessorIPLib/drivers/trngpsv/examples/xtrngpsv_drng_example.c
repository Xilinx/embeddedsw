...
#ifndef SDT
static int Trngpsv_Drng_Example(u16 DeviceId);
#else
static int Trngpsv_Drng_Example(UINTPTR BaseAddr);
#endif
...
/* Initial entropy contains 384bit seed */
...
int main(void)
{
	...
	xil_printf("********* TRNGPSV example in DRNG mode **********\n\r");
	Status = Trngpsv_Drng_Example(TRNGPSV_DEVICE);
	if (Status != XST_SUCCESS) {
		xil_printf("TRNGPSV DRNG example failed\n\r");
		return XST_FAILURE;
	}
	xil_printf("Successfully ran TRNGPSV DRNG example\n\r");
 ...
 ************************************************************************************************/
#ifndef SDT
int Trngpsv_Drng_Example(u16 DeviceId)
#else
int Trngpsv_Drng_Example(UINTPTR BaseAddr)
#endif
{
	...
	Status = Xil_SMemCpy(&UsrCfg.InitSeed, XTRNGPSV_SEED_LEN_BYTES, InitEntropy,
			sizeof(InitEntropy), sizeof(InitEntropy));
	if (Status != XST_SUCCESS) {
		goto END;
	}
	...
	/* Reseed with new entropy and DFLenMul of 0 */
	Status = XTrngpsv_Reseed(&Trngpsv, ReseedEntropy, 0U);
	if (Status != XST_SUCCESS) {
