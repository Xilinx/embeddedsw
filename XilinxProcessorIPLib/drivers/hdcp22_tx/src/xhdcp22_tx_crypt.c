...
*
* @param  KeyPubNPtr is the public certificate modulus.
* @param  KeyPubNSize is the size public certificate modulus.
* @param  KeyPubEPtr is the public certificate exponent.
* @param  KeyEPubSize is the public certificate exponent size.
...
/******************************************************************************
 * @brief Computes the V hash value for HDCP 2.2 transmitter authentication.
 *
 * This function calculates the V parameter used in HDCP 2.2 content protection
 * protocol for receiver authentication and repeater topology verification.
 *
 * @param Rn Pointer to the random nonce value.
 * @param Rrx Pointer to the receiver's random value.
 * @param RxInfo Pointer to the receiver information structure.
 * @param Rtx Pointer to the transmitter's random value.
 * @param RecvIDList Pointer to the list of connected receiver IDs.
 * @param RecvIDCount Number of receivers in the RecvIDList.
 * @param SeqNum_V Pointer to the sequence number for V computation.
 * @param Km Pointer to the master key used in computation.
 * @param V Pointer to the output buffer where computed V hash will be stored.
 *
 * @return None.
 *
 * @note All input pointers must be valid and point to properly initialized data.
 * @note The V output buffer must be pre-allocated with sufficient size.
 ******************************************************************************/
	...
	const u8* SeqNum_V, const u8* Km, u8* V)
{
 ...
 /******************************************************************************/
 /**
 * @brief Computes the M hash value for HDCP 2.2 authentication.
 *
 * This function calculates the M parameter used in the HDCP 2.2 protocol
 * by processing the input parameters through a cryptographic hash function.
 *
 * @param Rn Pointer to the Rn random number.
 * @param Rrx Pointer to the Rrx receiver random number.
 * @param Rtx Pointer to the Rtx transmitter random number.
 * @param StreamIDType Pointer to the Stream ID Type value.
 * @param k Pointer to the k parameter.
 * @param SeqNum_M Pointer to the sequence number for M.
 * @param Km Pointer to the Km master key.
 * @param M Pointer to the output buffer where computed M hash will be stored..
 *
 * @note The caller must ensure all input pointers are valid and point to
 *       buffers of appropriate size.
 * @note The output buffer M must be pre-allocated with sufficient size to
 *       store the computed hash value.
 *
 *****************************************************************************/
