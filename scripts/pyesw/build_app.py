...
class Build_App(BSP):
    ...
    def _build_dir_struct(self, args):
        if args["ws_dir"] == "." and not args.get("build_dir"):
            logger.warning(
                "The app Workspace is taken as current working directory. To avoid this, please use -w option"
            )
        ...
        self.domain_path = utils.fetch_yaml_data(self.app_config_file, "domain_path")[
            "domain_path"
        ]
...
def build_app(args):
    ...
    cmake_cache = os.path.join(
        obj.libsrc_folder, "build_configs", "gen_bsp", "CMakeCache.txt"
    )
    ...
    utils.copy_file(
        f"{obj.app_build_dir}/compile_commands.json",
        obj.app_src_dir,
        silent_discard=True,
    )
...
def main(arguments=None):
    ...
    parser.add_argument(
        "-s", "--src_dir", action="store", help="Specify the App source directory "
    )
    parser.add_argument(
