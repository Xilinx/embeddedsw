...
def retarget_app(args):
    ...
    current_domain_path = utils.fetch_yaml_data(app_config_file, "domain_path")[
        "domain_path"
    ]
    utils.update_yaml(
        app_config_file, "domain_path", "domain_path", obj.domain_path, action="add"
    )
    ...
    if current_domain_path != obj.domain_path:
        if args.get("build_dir"):
            ...
            if utils.is_dir(app_build_dir):
                logger.info(
                    f"Deleting existing build directory: {app_build_dir} (domain path mismatch detected)"
                )
        ...
        if utils.is_dir(app_build_dir):
            ...
            if utils.is_file(cmake_cache):
                ...
                if current_compiler_ar_path:
                    ...
                    current_compiler_ar_path = os.path.normpath(
                        current_compiler_ar_path
                    )
                    compiler_ar_name = os.path.basename(current_compiler_ar_path)
                    # Find the updated path
                    updated_compiler_ar_path = utils.find_compiler_path(
                        compiler_ar_name
                    )
                ...
                """
                logger.debug(
                    "No compile_commands.json found. Removing build directory for safety."
                )
                utils.remove(app_build_dir)
            ...
            if not utils.is_file(is_ninja_build):
                logger.debug(
                    "Ninja build file not found. Clearing build directory and updating CMakeLists.txt."
                )
                utils.remove(app_build_dir)
                src_cmake = os.path.join(obj.app_src_dir, "CMakeLists.txt")
                replace_header = f"""
