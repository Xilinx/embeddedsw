...
*
****************************************************************************/
static int XPlmi_IpiIntrHandler(void *CallbackRef)
{
	int Status = XST_FAILURE;
	XPlmi_TaskNode *Task = NULL;
	(void)CallbackRef;

	Task = XPlmi_GetTaskInstance(NULL, NULL, XPLMI_IPI_IRQ);
	if (Task == NULL) {
		XPlmi_Printf(DEBUG_GENERAL, "IPI Interrrupt add task error\n\r");
		goto END;
	}
...
int XPlmi_IpiInit(XPlmi_SubsystemHandler SubsystemHandler)
{
	...
	/* Create a task for IPI */
	Task = XPlmi_TaskCreate(XPLM_TASK_PRIORITY_0,
			XPlmi_IpiDispatchHandler, (void *)XPLMI_IPI_IRQ);
	if (Task == NULL) {
	...
	Task->IntrId = XPLMI_IPI_IRQ;
	Task->IsPersistent = (u8)TRUE;

	Status = XPlmi_RegisterHandler(XPLMI_IPI_IRQ, (GicIntHandler_t)XPlmi_IpiIntrHandler,
			(void *)0U);
...
END:
	...
	/* Clear and enable the IoModule IPI interrupt */
