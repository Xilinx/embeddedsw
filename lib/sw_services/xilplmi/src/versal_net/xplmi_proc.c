...
#define XPLMI_MAP_PLMID(Lvl0, Lvl1, Lvl2, Lvl3)	\
	(((Lvl0) << 0U) | ((Lvl1) << 8U) | ((Lvl2) << 16U) | ((Lvl3) << 24U))
...
static u32 PlmIntrMap[XPLMI_MAX_EXT_INTR] = {
	[XPLMI_CFRAME_SEU] = XPLMI_MAP_PLMID(XPLMI_IOMODULE_CFRAME_SEU,
					0x0U, 0x0U, 0x0U),
	[XPLMI_IPI_IRQ] = XPLMI_MAP_PLMID(XPLMI_IOMODULE_PMC_IPI,
					0x0U, 0x0U, 0x0U),
	[XPLMI_SBI_DATA_RDY] = XPLMI_MAP_PLMID(XPLMI_IOMODULE_PPU1_HW_INT,
					XPLMI_HW_INT_GIC_IRQ, XPLMI_PMC_GIC_IRQ_GICP6,
					XPLMI_GICP6_SRC1),
};
...
/*****************************************************************************/
/**
 * @brief	This is handler for PPU1 HW interrupts.
 *
 ...
 *
 *****************************************************************************/
void XPlmi_HwIntrHandler(void *CallbackRef) {
	u32 HwIntStatus;

	HwIntStatus = XPlmi_In32(PMC_GLOBAL_PPU1_HW_INT_ADDR);

	/* Call GIC Handler */
	if ((HwIntStatus & PMC_GLOBAL_PPU1_HW_INT_GIC_MASK) ==
			PMC_GLOBAL_PPU1_HW_INT_GIC_MASK) {
		XPlmi_GicIntrHandler(CallbackRef);
	}
	/* Call XPlmiIntrHandler if any other interrupt is set */
	if ((HwIntStatus & (~PMC_GLOBAL_PPU1_HW_INT_GIC_MASK)) != 0U){
		XPlmi_IntrHandler(CallbackRef);
	}

	/* Clear HwIntStatus */
	XPlmi_Out32(PMC_GLOBAL_PPU1_HW_INT_ADDR, HwIntStatus);
...
void XPlmi_PlmIntrEnable(u32 IntrId)
{
	...
	HwSwIntrNum = (PlmIntrId & XPLMI_HW_SW_INTR_MASK) >> XPLMI_HW_SW_INTR_SHIFT;

	switch (IoModIntrNum)
	{
		case XPLMI_IOMODULE_PPU1_HW_INT:
		...
		case XPLMI_IOMODULE_PMC_IPI:
			XIOModule_Enable(&IOModule, XPLMI_IOMODULE_PMC_IPI);
			break;
...
int XPlmi_PlmIntrClear(u32 IntrId)
{
	...
	switch (IoModIntrNum)
	{
		case XPLMI_IOMODULE_PPU1_HW_INT:
			...
			XPlmi_Out32(PMC_GLOBAL_PPU1_HW_INT_ADDR, 1U << HwSwIntrNum);
			Status = XST_SUCCESS;
			break;
