...
*      tnt  12/15/2021 Added support for psm_sequence CDO command
*      tnt  01/28/2022 Added support for scatter_write and scatter_write2
*
* </pre>
*
* @note
 ...
 *
 *****************************************************************************/
static int XPlmi_PsmSequence(XPlmi_Cmd *Cmd)
{
	int Status = XST_FAILURE;
	u32 ProcId;
	u8 Index = 0U;
	...
	if (Cmd->ProcessedLen == 0){
		...
		BeginAddr = ProcList->ProcData[ProcList->ProcCount].Addr;
		/* Check if new proc length fits in the proc allocated memory */
		if ((BeginAddr + (Cmd->Len * XPLMI_WORD_LEN)) > (XPLMI_PROC_LOCATION_ADDRESS +
				XPLMI_PROC_LOCATION_LENGTH)) {
	...
	if ((Cmd->ProcessedLen + Cmd->PayloadLen) == Cmd->Len ) {
		...
		Status = XPlmi_SendToPsm(BeginAddr, Cmd->Len);
		if (Status != XST_SUCCESS){
...
static int XPlmi_Begin(XPlmi_Cmd *Cmd)
{
	...
	if (Cmd->PayloadLen > 1U) {
		...
		Status = Xil_SecureMemCpy(LogString,
				XPLMI_BEGIN_MAX_LOG_STR_LEN,
				(u8 *)&Cmd->Payload[1U],
