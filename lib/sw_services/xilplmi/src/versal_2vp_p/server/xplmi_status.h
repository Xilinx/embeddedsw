...
/**
*
* @file versal_2vp/server/xplmi_status.h
*
* This is the header file which contains status codes for the PLM, PLMI and loader in versal_2vp
* platform.
*
* <pre>
 ...
 * XXXX - Major error code - PLM/LOADER/XPLMI error codes as defined in xplmi_status.h
 * YYYY - Minor error code - Libraries / Drivers error code as defined in respective modules
 */
...
/*
 * Use these masks to change any major error or minor error into a warning. It's expected to OR the
 * error code with any of these masks as required. This will set the 30th bit of Status which is
 * ultimately used to classify between error and warning. Note that this bit will be cleared before
...
/*
 * In case of failure of any security operation, the buffer must be cleared. In case of
 * success/failure in clearing the buffer, the following error codes shall be updated in the status
 */
...
typedef enum {
	...
	XPLMI_ERR_DMA_LOOKUP = 0x100,	/**< 0x100 - Error when DMA driver lookup fails. */
	XPLMI_ERR_DMA_CFG,		/**< 0x101 - Error when DMA driver config fails. */
	XPLMI_ERR_DMA_SELFTEST,		/**< 0x102 - Error when DMA Self test fails. It occurs when
					  DMA is in reset and PLM tries to initialize it */

	XPLMI_ERR_IOMOD_INIT,		/**< 0x103 - Error when IOModule driver look up fails */
	...
	XPLMI_ERR_CDO_HDR_ID,		/**< 0x10B - Error when valid CDO header ID is not present
					  in CDO header. Can happen when different partition type is
					  processed as CDO */
	...
	XPLMI_ERR_IPI_CMD,	/**< 0x115 - Error when command execution through IPI is not
				  supported because it's a CDO or due to access permission
				  restriction. See minor error code for more details */
	XPLMI_ERR_REGISTER_IOMOD_HANDLER,	/**< 0x116 - Error when registering IoModule
						  Handler */
	...
	XPLMI_ERR_NON_BLOCK_DMA_WAIT_SRC,	/**< 0x11E Non Block Dma transfer wait failed in Src
						  channel*/
	XPLMI_ERR_NON_BLOCK_DMA_WAIT_DEST,	/**< 0x11F Non Block Dma transfer wait failed in
						  Dest channel WaitForDone */
	XPLMI_ERR_NON_BLOCK_SRC_DMA_WAIT,	/**< 0x120 Non Block Src Dma transfer wait failed */
	XPLMI_ERR_NON_BLOCK_DEST_DMA_WAIT,	/**< 0x121 Non Block Dest Dma transfer wait
						  failed */
	...
	XPLMI_IPI_READ_ERR,	/**< 0x12E - Error in processing IPI request. It could be due to
				  invalid message length error when CRC is enabled or invalid buffer
	...
	XPLMI_ERR_INVALID_TASK_TYPE,	/**< 0x132 - Error when invalid task type is used to add
					  tasks in scheduler */
	XPLMI_ERR_INVALID_TASK_PERIOD,	/**< 0x133 - Error when invalid task period is used to add
					  tasks in scheduler */
	...
	XPLMI_ERR_SCHED_TASK_MISSED,	/**< 0x139 Scheduler task missed executing at the scheduled
					  interval */
	XPLMI_ERR_SET_PMC_IRO_FREQ,	/**< 0x13A - Error when setting PMC IRO frequency is
					  failed */
	...
	XPLMI_INVALID_BREAK_LENGTH,	/**< 0x140 - Error when the break length required to jump is
					  less than the processed CDO length */
	XPLMI_ERR_MODULE_NOT_REGISTERED,	/**< 0x141 - Error when the module of the CDO/IPI
						  command used is not registered */
	...
	XPLMI_SSIT_EVENT_VECTOR_TABLE_IS_FULL = 0x1A0,	/**< 0x1A0 - Error when the SSIT event
							  vector table is full and PLM is trying to
							  register a new event */
	XPLMI_SSIT_WRONG_EVENT_ORIGIN_MASK,	/**< 0x1A1 - Error when the event origin is not
						  valid */
	XPLMI_INVALID_SLR_TYPE,	/**< 0x1A2 - Error when any SSIT event related APIs are called from
				  SLR where it is not supported */
	XPLMI_INVALID_SLR_INDEX,	/**< 0x1A3 - Error when the given SlrIndex is invalid */
	XPLMI_EVENT_NOT_SUPPORTED_BETWEEN_SLAVE_SLRS,	/**< 0x1A4 - Error when the request to
							  trigger an event between slave slrs which
							  is not supported */
	XPLMI_SSIT_INVALID_EVENT,	/**< 0x1A5 - Error when request is received to trigger an
					  event which is not registered with PLM */
	XPLMI_SSIT_EVENT_IS_PENDING,	/**< 0x1A6 - Error when an event is pending and the request
					  comes to trigger the same again */
	XPLMI_SSIT_EVENT_NOT_ACKNOWLEDGED,	/**< 0x1A7 - Error when the triggered event is not
						  acknowledged within given TimeOut value */
	XPLMI_SSIT_BUF_SIZE_EXCEEDS,	/**< 0x1A8 - Error when SSIT request or response buffer size
					  exceeds */
	XPLMI_EVENT_NOT_SUPPORTED_FROM_SLR,	/**< 0x1A9 - Error when the given event is not
						  supported to be triggered from the running SLR */
	XPLMI_SSIT_EVENT_IS_NOT_PENDING,	/**< 0x1AA - Error when an event is not pending and
						  the request comes to write to the response
						  buffer */
	...
	XPLMI_SSIT_SECURE_COMM_FAIL_AT_MASTER_ENCRYPTION,	/**< 0x1AF - Error if secure plm to
								  plm communication fails at
								  encryption on master side*/
	XPLMI_SSIT_SECURE_COMM_FAIL_AT_MASTER_DECRYPTION,	/**< 0x1B0 - Error if secure plm to
								  plm communication fails at
								  decryption on master side*/
	XPLMI_SSIT_SECURE_COMM_FAIL_AT_SLAVE_ENCRYPTION,	/**< 0x1B1 - Error if secure plm to
								  plm communication fails at
								  encryption on slave side*/
	XPLMI_SSIT_SECURE_COMM_FAIL_AT_SLAVE_DECRYPTION,	/**< 0x1B2 - Error if secure plm to
								  plm communication fails at
								  decryption on slave side*/
	...
	XPLMI_ERR_MEMSET_DBHDR,		/**< 0x1BB - Error when memset of DbHdr during store database is failed */
	XPLMI_ERR_MEMCPY_RELOCATE,	/**< 0x1BC - Error when relocating of update manager code is failed */
	XPLMI_ERR_PLM_UPDATE_RELOCATED_FN, /**< 0x1BD - Error when the relocated PLM update
					     function fails */
	XPLMI_ERR_PLM_UPDATE_SHUTDOWN_COMPLETE, /**< 0x1BE - Error in shutdown complete of modules
						during InPlace PLM Update */
	XPLMI_ERR_RETRY_SHUTDOWN_LATER, /**< 0x1BF - Error when a module cannot be shutdown currently and request to
						retry later */
	XPLMI_ERR_STORE_DATA_BACKUP,	/**< 0x1C0 - Error when data structure storing fails */
	XPLMI_ERR_INVALID_STORE_DS_HANDLER, /**< 0x1C1 - Error due to invalid Data Structure Handler used
						in storing of Data Structures during InPlace PLM Update */
	XPLMI_ERR_INVALID_RESTORE_DS_HANDLER, /**< 0x1C2 - Error due to invalid Data Structure Handler used
						in restoring of Data Structures during InPlace PLM Update */

	XPLMI_ERR_INVALID_DS_ENTRY, 	/**< 0x1C3 - Error when a invalid Data Structure entry is passed
						to the PLM Db Update Handler */
	XPLMI_ERR_DS_ALIGNMENT_INCORRECT, /**< 0x1C4 - Error when the alignment of Data Structure used during
						store or restore operations is not word aligned Header */
	XPLMI_ERR_PLM_UPDATE_DB_OVERFLOW, /**< 0x1C5 - Error when the given Data Structure length is exceed
						the Update DB buffer available */
	XPLMI_ERR_PLM_UPDATE_INVALID_OP, /**< 0x1C6 - Error when a invalid operation is passed to PLM
						Update DB Handler */
	XPLMI_ERR_DB_HDR_SIZE_MISMATCH,	/**< 0x1C7 - Error when the DB Hdr size of old PLM is not matching
						with the size of updated PLM */
	XPLMI_ERR_DB_HDR_VERSION_MISMATCH, /**< 0x1C8 - Error when the DB Hdr version of old PLM is not matching
						with that of updated PLM */
	XPLMI_ERR_DB_ENDADDR_INVALID, /**< 0x1C9 - Error when the DB end address calculated is not in a
	...
	XLOADER_UNSUPPORTED_BOOT_MODE = 0x300,	/**< 0x300 - Error for unsupported bootmode. It
	...
	XLOADER_ERR_PRTNHDR,	/**< 0x304 - Error if partition header checksum fails */
	XLOADER_ERR_WAKEUP_A72_0,	/**< 0x305 - Error waking up the A72-0 during handoff. Check
					  the PLM minor code for PM error code. */
	XLOADER_ERR_WAKEUP_A72_1,	/**< 0x306 - Error waking up the A72-1 during handoff. Check
					  the PLM minor code for PM error code. */
	XLOADER_ERR_WAKEUP_R5_0,	/**< 0x307 - Error waking up the R5-0 during handoff. Check
					  the PLM minor code for PM error code. */
	XLOADER_ERR_WAKEUP_R5_1,	/**< 0x308 - Error waking up the R5-1 during handoff. Check
					  the PLM minor code for PM error code. */
	XLOADER_ERR_WAKEUP_R5_L,	/**< 0x309 - Error waking up the R5-L during handoff. Check
					  the PLM minor code for PM error code. */
	...
	XLOADER_ERR_UNSUPPORTED_OSPI_SIZE,	/**< 0x30D - Error for unsupported OSPI flash
						  size */
	XLOADER_ERR_OSPI_INIT,	/**< 0x30E - Error when OSPI driver lookup fails. This happens when
				  OSPI is not selected in CIPS */
	XLOADER_ERR_OSPI_CFG,	/**< 0x30F - Error when OSPI driver CFG fails */
	XLOADER_ERR_OSPI_SEL_FLASH_CS0,	/**< 0x310 - Error when OSPI driver is unable to select
					  flash CS0. Check minor code for OSPI driver error code */
	XLOADER_ERR_OSPI_READID,	/**< 0x311 - Error when OSPI ReadID fails */
	XLOADER_ERR_OSPI_READ,	/**< 0x312 - Error when OSPI driver read fails. Check minor code for
				  OSPI driver error code */
	...
	XLOADER_ERR_QSPI_READ,	/**< 0x31A - Error when QSPI driver Read fails */
	XLOADER_ERR_QSPI_LENGTH,	/**< 0x31B - Error when QSPI read length is greater than
					  flash size */

	XLOADER_ERR_SD_INIT,	/**< 0x31C - Error when SD mount fails*/
	XLOADER_ERR_SD_F_OPEN,	/**< 0x31D - Error when SD file open fails. This can happen when
				  file is not present or read from SD fails.  File system error code
				  is present in PLM minor code */
	XLOADER_ERR_SD_F_LSEEK,	/**< 0x31E - Error when f_seek fails while reading from SD card */
	XLOADER_ERR_SD_F_READ,	/**< 0x31F - Error while reading from SD card */

	XLOADER_ERR_IMG_ID_NOT_FOUND,	/**< 0x320 - Error when Image ID is not found in subsystem
					  while reloading image */
	XLOADER_ERR_TCM_ADDR_OUTOF_RANGE,	/**< 0x321 - Error while loading to TCM and if
						  address is out of range*/
	XLOADER_ERR_CFRAME_LOOKUP,	/**< 0x322 - Error when CFRAME driver look up fails */
	XLOADER_ERR_CFRAME_CFG,	/**< 0x323 - Error when CFRAME driver CFG fails */
	XLOADER_ERR_UNSUPPORTED_SEC_BOOT_MODE,	/**< 0x324 - Error for unsupported secondary boot
						  mode */
	XLOADER_ERR_SECURE_METAHDR,	/**< 0x325 - Error when meta header secure validations
					  fail. */
	XLOADER_ERR_GEN_IDCODE,	/**< 0X326 - Error if Vivado configured part (IDCODE) mismatches
	...
	XLOADER_ERR_DEFERRED_CDO_PROCESS,	/**< 0x32B - Error if the expected value is not same
						  ...
						  configured to defer it till the whole CDO
	...
	XLOADER_ERR_MMC_PART_CONFIG,	/**< 0x32F - Error when MMC part configuration fails */
	XLOADER_ERR_SEM_STOP_SCAN,	/**< 0x330 - Error while stopping the SEM Scan */
	XLOADER_ERR_SEM_INIT,	/**< 0x331 - Error while starting the SEM Scan */
	XLOADER_ERR_DELAY_ATTRB,	/**< 0x332 - Error when both delay handoff and delay load
					  are set for the same image */
	XLOADER_ERR_NUM_HANDOFF_CPUS,	/**< 0x333 - Error when number of CPUs exceed max count */
	XLOADER_ERR_OSPI_CONN_MODE,	/**< 0x334 - Error when OSPI mode is not supported */
	XLOADER_ERR_OSPI_SEL_FLASH_CS1,	/**< 0x335 - Error when OSPI driver is unable to select
					  flash CS1. Check minor code for OSPI driver error code */
	XLOADER_ERR_OSPI_SDR_NON_PHY,	/**< 0x336 - Error when OSPI driver is unable to set the
					  controller to SDR NON PHY mode */
	XLOADER_ERR_OSPI_COPY_OVERFLOW,	/**< 0x337 - Error when source address in OSPI copy exceeds
					  flash size */
	...
	XLOADER_ERR_DELAY_LOAD,	/**< 0x33E - When the image has delay load attribute set and the
				  boot source is SMAP, SBI, PCIE or JTAG, the image is copied to PMC
				  RAM to free it from the SBI buffers. Errors occurred during such
				  copies to PMC RAM is denoted using this error code. */
	XLOADER_ERR_ADD_TASK_SCHEDULER,	/**<0x33F - Error while adding task to the scheduler */
	XLOADER_ERR_SD_MAX_BOOT_FILES_LIMIT,	/**< 0x340 - Error code returned when search for
						  bootable file crosses max limit */
	...
	XLOADER_ERR_PM_DEV_PSM_PROC,	/**< 0x342 - Failed in XPM Request Device for
					  PM_DEV_PSM_PROC */
	XLOADER_ERR_PM_DEV_IOCTL_RPU0_SPLIT,	/**<0x343 - Failed in XPM Device Ioctl for RPU0_0
						  in SPLIT mode */
	XLOADER_ERR_PM_DEV_IOCTL_RPU1_SPLIT,	/**< 0x344 - Failed in XPM Device Ioctl for RPU0_1
						  in SPLIT mode */
	XLOADER_ERR_PM_DEV_IOCTL_RPU0_LOCKSTEP,	/**< 0x345 - Failed to XPM Device Ioctl for RPU0_0
						  in LOCKSTEP mode */
	XLOADER_ERR_PM_DEV_IOCTL_RPU1_LOCKSTEP,	/**< 0x346 - Failed to XPM Device Ioctl for RPU0_1
						  in LOCKSTEP mode */
	...
	XLOADER_ERR_PM_DEV_DDR_0,	/**< 0x34B - Failed to XPM Request Device for
					  PM_DEV_DDR_0 */
	XLOADER_ERR_PM_DEV_QSPI,	/**< 0x34C - Failed to XPM Request Device for PM_DEV_QSPI */
	...
	XLOADER_ERR_DEV_NOT_DEFINED,	/**< 0x351 - Device ID of the image to be loaded is not
					  defined */
	XLOADER_ERR_PARENT_QUERY_VERIFY,	/**< 0x352 - Failed to Query Parent ID of an image
						  while verifying its Image UIDs */
	XLOADER_ERR_INCOMPATIBLE_CHILD_IMAGE,	/**< 0x353 - Error while checking compatibility of a
						  image with it's parent */
	XLOADER_ERR_NO_VALID_PARENT_IMG_ENTRY,	/**< 0x354 - Error if No Valid Parent Image Entry is
						  found in the ImageInfo Table */
	...
	XLOADER_ERR_IMAGE_INFO_TBL_OVERFLOW,	/**< 0x357 - Error when ImageInfo Table overflows */
	XLOADER_ERR_FUNCTION_ID_MISMATCH,	/**< 0x358 - Error when Function ID given while
						  loading Image from DDR is not matching with the ID
						  stored in Image Header */
	XLOADER_ERR_MEMSET,	/**< 0x359 - Error during memset */
	XLOADER_DDR_COPY_UNSUPPORTED_PARAMS,	/**< 0x35A - Error when source address, destination
						  address or length params passed to XLoader_DdrCopy
						  are not word aligned */
	XLOADER_ERR_INIT_CDO,	/**< 0x35B - XPlmi_InitCdo failed */
	XLOADER_ERR_INVALID_ELF_LOAD_ADDR,	/**< 0x35C - Error when the load address of the elf
						  is not valid */
	XLOADER_ERR_UNSUPPORTED_MULTIBOOT_FLASH_TYPE,	/**<0X35D - Error due to unsupported Flash
							  Type used with Update Multiboot command */
	XLOADER_ERR_UNSUPPORTED_MULTIBOOT_PDISRC,	/**< 0x35E - Error due to unsupported PdiSrc
							  used with Update Multiboot command */
	XLOADER_ERR_UNSUPPORTED_FILE_NUM,	/**< 0x35F - Error due to unsupported Filenum used
						  to update multiboot register */
	XLOADER_ERR_UNSUPPORTED_MULTIBOOT_OFFSET,	/**< 0x360 - Error when given multiboot
							  offset is not valid (not a multiple of
							  32K) */
	XLOADER_ERR_SECURE_NOT_ENABLED,	/**< 0x361 - Error as secure critical code is excluded and
					  Secure boot is attempted */
	XLOADER_ERR_UNSUPPORTED_SUBSYSTEM_PDISRC,	/**< 0x362 - Error when unsupported PdiSrc
							  is used for subsystem load */
	...
	XLOADER_ERR_INVALID_PRTNCOPY_DEST_ADDR,	/**< 0x375 - Invalid Destination address in PrtnCopy
						  API for Raw Partition Loading */
	...
	XLOADER_ERR_INIT_GET_DMA = 0x600,	/**< 0x600 Failed to get DMA instance at time of
						  initialization */
	XLOADER_ERR_INIT_INVALID_CHECKSUM_TYPE,	/**< 0x601 only SHA3 checksum is supported */
	XLOADER_ERR_INIT_CHECKSUM_COPY_FAIL,	/**< 0x602 Failed when copying Checksum from flash
						  device */
	XLOADER_ERR_INIT_AC_COPY_FAIL,	/**< 0x603 Failed when copying AC from flash device */
	XLOADER_ERR_INIT_CHECKSUM_INVLD_WITH_AUTHDEC,	/**< 0x604 Failed as checksum was enabled
	...
	XLOADER_ERR_IHT_HASH_CALC_FAIL,	/**< 0x609 Failed to calculate hash for IHT
					  authentication */
	XLOADER_ERR_IHT_AUTH_FAIL,	/**< 0x60A Failed to authenticate IHT */

	XLOADER_ERR_HDR_COPY_FAIL,	/**< 0x60B Failed when copying IH/PH from flash device */
	XLOADER_ERR_HDR_AES_OP_FAIL,	/**< 0x60C Failed due to AES init or Decrypt init or key
					  selection failure */
	XLOADER_ERR_HDR_DEC_FAIL,	/**< 0x60D Failed to decrypt IH/PH */
	XLOADER_ERR_HDR_AUTH_FAIL,	/**< 0x60E Failed to authenticate IH/PH */
	XLOADER_ERR_HDR_NOT_SECURE,	/**< 0x60F Neither authentication nor encryption enabled for
					  IH/PH */
	XLOADER_ERR_HDR_GET_DMA,	/**< 0x610 Failed to get DMA instance for IH/PH
	...
	XLOADER_ERR_PRTN_HASH_CALC_FAIL,	/**< 0x616 Hash calculation failed for partition
						  authentication */
	XLOADER_ERR_PRTN_AUTH_FAIL,	/**< 0x617 Partition authentication failed */
	XLOADER_ERR_PRTN_HASH_COMPARE_FAIL,	/**< 0x618 Partition hash comparison failed */
	XLOADER_ERR_PRTN_DECRYPT_FAIL,	/**< 0x619 Partition decryption failed */
	...
	XLOADER_ERR_KAT_FAILED,	/**< 0x61E KAT failed */
	XLOADER_ERR_DATA_COPY_FAIL,	/**< 0x61F Data copy to internal memory failed */
	XLOADER_ERR_METAHDR_LEN_OVERFLOW,	/**< 0x620 Failed when total size is greater than
						  Metahdr length */

	XLOADER_ERR_AUTH_JTAG_EFUSE_AUTH_COMPULSORY,	/**< 0x621 Jtag Authentication failed when
							  PPK not programmed */
	XLOADER_ERR_AUTH_JTAG_DISABLED,	/**< 0x622 Jtag Authentication disable efuse bit is set */
	XLOADER_ERR_AUTH_JTAG_PPK_VERIFY_FAIL,	/**< 0x623 Jtag Authentication failed when
	...
	XLOADER_ERR_AUTH_JTAG_DMA_XFR,	/**< 0x628 Failed to get Auth Jtag data with DMA Xfr */
	XLOADER_ERR_MEMSET_SECURE_PTR,	/**< 0x629 Error during memset for SecurePtr */
	XLOADER_ERR_GLITCH_DETECTED,	/**< 0x62A Error glitch detected */
	XLOADER_ERR_AUTH_JTAG_SPK_REVOKED,	/**< 0X62B Jtag Authentication failed when revoke id
						  is programmed */
	XLOADER_ERR_METAHDR_KEYSRC_MISMATCH,	/**< 0x62C Metaheader Key Source does not match PLM
						  Key Source */
	...
	XLOADER_ERR_UNSUPPORTED_PDI_VER,	/**< 0x630 PDI version used in secure operations is
						  unsupported */
	XLOADER_ERR_PRTN_DECRYPT_NOT_ALLOWED,	/**< 0x631 Partition is not allowed to be encrypted
						  if State of boot is non secure */
	XLOADER_ERR_AUTH_JTAG_INVALID_DNA,	/**< 0x632 User provided Device DNA is not valid */
	XLOADER_ERR_SEC_IH_VERIFY_FAIL,	/**< 0x633 Failed to verify checksum of image headers */
	XLOADER_ERR_SEC_PH_VERIFY_FAIL,	/**< 0x634 Failed to verify checksum of partition headers */
	XLOADER_ERR_SECURE_CLEAR_FAIL,	/**< 0x635 Failed to place either AES,RSA,SHA3 engine in
					  reset */
	...
	XLOADER_PRTN_HASH_NOT_PRESENT,	/**< 0x63B Partition hash not present, data measurement not
					  possible */
	XLOADER_ERR_SECURE_CONFIG_MEASUREMENT,    /**< 0x63C - Error in Secure config measurement */

	XLOADER_ERR_HASH_BLOCK_COPY_FAIL, /**< 0x63D - Error in HashBlock copy */
	XLOADER_ERR_HASH_BLOCK_HASH_CALC_FAIL, /**< 0x63E - Error in calculating HashBlock*/
	XLOADER_ERR_HASH_BLOCK_HASH_COMPARE_FAIL,
					/**< 0x63F - Error in HashBlock hash comparison */
	XLOADER_ERR_MH_HASH_CALC_FAIL, /**< 0x6340 - Error in calculating MetaHeader hash */
	XLOADER_ERR_META_HDR_HASH_COMPARE_FAIL,
					/**< 0x641 - Error in MetaHeader hash comparison */
	XLOADER_ERR_HASH_BLOCK_AAD_VALIDATE, /**< 0x642 - Error in HashBlock AAD validation */
	XLOADER_ERR_PPK_HASH_CALC_FAIL, /**< 0x643 - Error in PPK hash calculation */
	XLOADER_ERR_SPK_HASH_CALC_FAIL, /**< 0x644 - Error in SPK hash calculation */
	XLOADER_ERR_PPK_COPY_FAIL, /**< 0x645 - Error in PPK copy */
	XLOADER_ERR_SPK_HEADER_COPY_FAIL, /**< 0x646 - Error in SPK Header copy */
	XLOADER_ERR_SPK_HEADER_VALIDATE_FAIL, /**< 0x647 - Error in SPK Header validation
*/
	XLOADER_ERR_SPK_COPY_FAIL, /**< 0x648 - Error in SPK copy */
	XLOADER_ERR_SPK_SIGN_COPY_FAIL, /**< 0x649 - Error in SIGN SIGN copy */
	...
	XPLMI_ERR_USER_MODULE_CDO_CMD = 0x3000,	/**< 0x3XXX, CDO command handler has failed for user
						  modules. [12:8] contains Module ID, [7:0] contains
						  API ID. Refer Minor code for Handler error code */
} XPlmiStatus_t;
