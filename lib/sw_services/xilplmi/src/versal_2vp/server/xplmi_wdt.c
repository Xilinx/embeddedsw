...
/**
*
* @file versal_2vp/server/xplmi_wdt.c
*
* This file contains the PLMI WDT functionality related code for versal_2vp.
*
* <pre>
...
/**************************************************************************************************/
/**
 * @brief	This function enables the WDT and sets NodeId and periodicity. It also verifies the
 * 		parameters.
 *
...
int XPlmi_EnableWdt(u32 NodeId, u32 Periodicity)
{
	...
	if ((NodeId >= XPLMI_PM_STMIC_LMIO_0) && (NodeId <= XPLMI_PM_STMIC_LMIO_25)) {
		/* LPD MIO is used */
	...
	} else if ((NodeId >= XPLMI_PM_STMIC_PMIO_0) && (NodeId <= XPLMI_PM_STMIC_PMIO_51)) {
		/* PMC MIO used */
		...
		WdtInstance.GpioMask = (u32)(1U) << (MioNum % XPLMI_MIO_NUM_PER_BANK);
	} else {
...
/**************************************************************************************************/
/**
 * @brief	This function is handler for WDT. Scheduler calls this function	periodically to
 * 		check the PLM Live status and to toggle the MIO.
 *
 **************************************************************************************************/
void XPlmi_WdtHandler(void)
{
	...
	if (WdtLastResetPeriod > (u32)(WdtInstance.Periodicity - XPLMI_WDT_PERIODICITY_MIN)) {
