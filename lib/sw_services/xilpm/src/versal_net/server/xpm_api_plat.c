...
XStatus XPm_PlatRequestDevice(const u32 DeviceId)
{
	XStatus Status = XST_FAILURE;
	...
	if(XPM_NODETYPE_DEV_CORE_RPU == NODETYPE(DeviceId)){
		...
		u32 RstRpuMask = 0U;
		u64 Tcm_Addr = (DeviceId == PM_DEV_RPU_A_0) ||
				(DeviceId == PM_DEV_RPU_B_0) ? XPM_R52_0A_TCMA_BASE_ADDR:
						XPM_R52_1A_TCMA_BASE_ADDR;
		u8 Ecc_Mask = (DeviceId == PM_DEV_RPU_A_0) ||
			       (DeviceId == PM_DEV_RPU_B_0)?XPM_R52_0_TCMA_ECC_DONE:
		...
		LockstepVal = XPm_In32((RPU_CLUSTER_BASEADDR +
					(ClusterNum * RPU_CLUSTER_OFFSET) +
					XPM_RPU_CLUSTER_CFG_OFFSET)) & 0x1;
