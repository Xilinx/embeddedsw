...
static XPm_Iso XPmDomainIso_List[XPM_NODEIDX_ISO_MAX] = {
	...
	[XPM_NODEIDX_ISO_LPD_CPM6_DFX] = {
		.Node.Id = ISOID(XPM_NODEIDX_ISO_LPD_CPM6_DFX),
	...
	[XPM_NODEIDX_ISO_LPD_CPM6] = {
		.Node.Id = ISOID(XPM_NODEIDX_ISO_LPD_CPM6),
		.Node.BaseAddress = CPM6_SLCR_BASEADDR,
		.Node.State = (u8)PM_ISOLATION_ON,
		.Mask = 0U,
		.Polarity = (u8)PM_ACTIVE_HIGH,
...
void XPmDomainIso_CpmPcieIsoRemoval(void)
{
	...
	if (XPmDomainIso_List[XPM_NODEIDX_ISO_PL_CPM_PCIEA0_ATTR].Node.State == (u8)PM_ISOLATION_REMOVE_PENDING) {
		DisableCpmPcieIso((u32)XPM_NODEIDX_ISO_PL_CPM_PCIEA0_ATTR);
	}
