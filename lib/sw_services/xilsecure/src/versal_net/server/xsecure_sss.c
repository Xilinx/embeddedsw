...
static const u8 XSecure_SssLookupTable
	...
	/*+----+-----+-----+-----+-----+-----+-----+--------+
	*|DMA0| DMA1| PTPI| AES | SHA | SBI | SHA1 |Invalid |
	*+----+-----+-----+-----+-----+-----+-----+--------+
	* 0x00 = INVALID value
	*/
	...
	{0x0CU, 0x07U, 0x00U, 0x00U, 0x00U, 0x00U, 0x00U, 0x00U}, /* SHA0 */
	{0x05U, 0x0BU, 0x00U, 0x00U, 0x00U, 0x00U, 0x00U, 0x00U}, /* SBI  */
	{0x0AU, 0x0FU, 0x00U, 0x00U, 0x00U, 0x00U, 0x00U, 0x00U}, /* SHA1 */
 ...
 *
 *****************************************************************************/
int XSecure_SssSha(const XSecure_Sss *InstancePtr, u16 DmaId, XSecure_SssSrc Resource)
{
	int Status = XST_FAILURE;
							...
							u32 Value)
{
	...
	if ((InputSrc == XSECURE_SSS_DMA0) || (OutputSrc == XSECURE_SSS_DMA0)) {
		...
		if ((RegVal & XSECURE_SSS_SHA_0_MASK) == XSECURE_SSS_SHA_0_DMA0_VAL) {
			Mask |= XSECURE_SSS_SHA_0_MASK;
		}
		if ((RegVal & XSECURE_SSS_AES_MASK) == XSECURE_SSS_AES_DMA0_VAL) {
			Mask |= XSECURE_SSS_AES_MASK;
		}
		if ((RegVal & XSECURE_SSS_SHA_1_MASK) == XSECURE_SSS_SHA_1_DMA0_VAL) {
			Mask |= XSECURE_SSS_SHA_1_MASK;
		}
	...
	if ((InputSrc == XSECURE_SSS_DMA1) || (OutputSrc == XSECURE_SSS_DMA1)) {
		...
		if ((RegVal & XSECURE_SSS_SHA_0_MASK) == XSECURE_SSS_SHA_0_DMA1_VAL) {
			Mask |= XSECURE_SSS_SHA_0_MASK;
		}
		if ((RegVal & XSECURE_SSS_AES_MASK) == XSECURE_SSS_AES_DMA1_VAL) {
			Mask |= XSECURE_SSS_AES_MASK;
		}
		if ((RegVal & XSECURE_SSS_SHA_1_MASK) == XSECURE_SSS_SHA_1_DMA1_VAL) {
			Mask |= XSECURE_SSS_SHA_1_MASK;
		}
