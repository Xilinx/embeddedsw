 ...
 * @param	DeviceId	- Device id to select SHA0 or SHA1
 *
 * @return	- XST_SUCCESS - If initialization was successful
 *		- XSECURE_SHA3_INVALID_PARAM - On invalid parameter
 *
 *****************************************************************************/
int XSecure_Sha3Initialize(XSecure_Sha3 *InstancePtr, XPmcDma* DmaPtr, u8 DeviceId)
{
	...
	if ((DeviceId != (u8)XSECURE_SHA3_0_DEVICE_ID) &&
		(DeviceId != (u8)XSECURE_SHA3_1_DEVICE_ID)) {
		Status = (int)XSECURE_SHA3_INVALID_PARAM;
		goto END;
	}

	InstancePtr->DeviceId = DeviceId;
	if (DeviceId == XSECURE_SHA3_0_DEVICE_ID) {
		InstancePtr->BaseAddress = XSECURE_SHA3_0_BASE_ADDRESS;
	}
	else {
		InstancePtr->BaseAddress = XSECURE_SHA3_1_BASE_ADDRESS;
	}
	...
	u64 InDataAddr, const u32 Size, u8 IsLastUpdate)
{
	...
	if (InstancePtr->DeviceId == XSECURE_SHA3_0_DEVICE_ID) {
		Status = XSecure_SssSha(&(InstancePtr->SssInstance),
					InstancePtr->DmaPtr->Config.DeviceId, XSECURE_SSS_SHA0);
	}
	else {
		Status = XSecure_SssSha(&(InstancePtr->SssInstance),
					InstancePtr->DmaPtr->Config.DeviceId, XSECURE_SSS_SHA1);
	}
	if (Status != XST_SUCCESS) {
		goto ENDF;
	}
