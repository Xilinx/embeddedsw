...
/************************************ Function Prototypes *****************************************/
#ifndef PLM_RSA_EXCLUDE
static int XSecure_RsaDecrypt(u32 SrcAddrLow, u32 SrcAddrHigh, u32 DstAddrLow, u32 DstAddrHigh);
#endif
...
/**************************************************************************************************/
/**
 * @brief	This function handler calls XSecure_RsaInitialize and XSecure_RsaPrivateDecrypt
 * 		server APIs.
 *
 * @param	SrcAddrLow	Lower 32 bit address of the XSecure_RsaInParam structure.
 * @param	SrcAddrHigh	Higher 32 bit address of the XSecure_RsaInParam structure.
 * @param	DstAddrLow	Lower 32 bit address of the output data where decrypted data to be
 * 				stored.
 * @param	DstAddrHigh	Higher 32 bit address of the output data where decrypted data to be
 * 				stored.
 *
 * @return
...
static int XSecure_RsaDecrypt(u32 SrcAddrLow, u32 SrcAddrHigh, u32 DstAddrLow, u32 DstAddrHigh)
{
	...
	Status = XSecure_RsaInitialize_64Bit(XSecureRsaInstPtr, Modulus, 0U, PrivateExp);
	if (Status != XST_SUCCESS) {
		goto END;
	}

	Status = XST_FAILURE;
	Status = XSecure_RsaPrivateDecrypt_64Bit(XSecureRsaInstPtr, RsaParams.DataAddr,
						 RsaParams.Size, DstAddr);
