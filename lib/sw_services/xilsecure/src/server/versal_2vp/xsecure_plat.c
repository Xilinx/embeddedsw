...
/**
 *
 * @brief	This function validates whether size of the data is word aligned and if it is the
 * 		last chunk size should be qword aligned.
 *
 ...
 * @param	Size		Size of data in bytes.
 * @param	BaseAddress	Not applicable for versal_2vp.
 *
 * @return
		...
		u32 Size, UINTPTR BaseAddress)
{
	...
	if ((AesDmaCfg->DestChannelCfg == TRUE) &&
		((u32)AesDmaCfg->DestDataAddr != XSECURE_AES_NO_CFG_DST_DMA)) {
		XPmcDma_64BitTransfer(PmcDmaPtr, XPMCDMA_DST_CHANNEL, (u32)AesDmaCfg->DestDataAddr,
				(u32)(AesDmaCfg->DestDataAddr >> XSECURE_ADDR_HIGH_SHIFT),
				Size / XSECURE_WORD_SIZE, AesDmaCfg->IsLastChunkDest);
	}

	if (AesDmaCfg->SrcChannelCfg == TRUE) {
		XPmcDma_64BitTransfer(PmcDmaPtr, XPMCDMA_SRC_CHANNEL, (u32)AesDmaCfg->SrcDataAddr,
				(u32)(AesDmaCfg->SrcDataAddr >> XSECURE_ADDR_HIGH_SHIFT),
