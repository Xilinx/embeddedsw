...
/**
 * levels = 1
 * LMS type = XSECURE_LMS_SHAKE_M32_H5
 * OTS type = XSECURE_LMS_OTS_SHAKE_N32_W2
 */
...
/**
 * levels = 2
 * LMS type = XSECURE_LMS_SHA256_M32_H5
 * OTS type = XSECURE_LMS_OTS_SHA256_N32_W2
 */
...
int XSecure_HmacKat(XSecure_Sha *SecureSha)
{
	...
	Status = XSecure_HmacInit(&HmacInstance, SecureSha, (UINTPTR)HmacKey,
...
int XSecure_RsaPrivateDecryptKat(void)
{
	...
	Status = XSecure_RsaInitialize(&XSecureRsaInstance, (u8 *)RsaModulus, (u8 *)RsaModExt,
					(u8 *)RsaPrivateExp);
	if (Status != XST_SUCCESS) {
...
END_RST:
	...
	XSecure_SetReset(SecureSha2->BaseAddress, XSECURE_SHA_RESET_OFFSET);
...
int XSecure_ShakeKat(XSecure_Sha *SecureShake)
{
	...
	Status = (int)XST_FAILURE;
	Status = XSecure_ShaUpdate(SecureShake, (UINTPTR)KatMessage, 32U);
	if (Status != XST_SUCCESS) {
		Status = (int)XST_FAILURE;
		goto END_RST;
	}
...
END_RST:
	...
	XSecure_SetReset(SecureShake->BaseAddress, XSECURE_SHA_RESET_OFFSET);
...
int XSecure_HssSha2256Kat(XSecure_Sha *ShaInstPtr, XPmcDma *DmaPtr)
{
	...
	Status = XSecure_LmsHashMessage(ShaInstPtr, HssSha2_256LmsData, XSECURE_LMS_DATA_SIZE_BYTES,
					XSECURE_SHA2_256);
	if (Status != XST_SUCCESS) {
		Status = (int)XSECURE_HSS_SHA2_256_KAT_ERROR;
		goto END;
	...
	Status = XSecure_HssFinish(ShaInstPtr, DmaPtr, Signature_HssSha2256_l0h5w2,
					XSECURE_HSS_SIGNATURE_SIZE_BYTES);
	if (Status != XST_SUCCESS) {
		Status = (int)XSECURE_HSS_SHA2_256_KAT_ERROR;
		goto END;
	}
...
int XSecure_HssShake256Kat(XSecure_Sha *ShaInstPtr, XPmcDma *DmaPtr)
{
	...
	Status = XSecure_LmsHashMessage(ShaInstPtr, Shake_256HssData, XSECURE_LMS_DATA_SIZE_BYTES,
					XSECURE_SHAKE_256);
	if(Status != XST_SUCCESS) {
		Status = (int)XSECURE_HSS_SHAKE_256_KAT_ERROR;
		goto END;
	...
	Status = XSecure_HssFinish(ShaInstPtr, DmaPtr, Signature_Shake256_l0h5w2,
					XSECURE_HSS_SIGNATURE_SIZE_BYTES);
	if (Status != XST_SUCCESS) {
		Status = (int)XSECURE_HSS_SHAKE_256_KAT_ERROR;
		goto END;
