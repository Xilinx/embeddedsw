...
static int XSecure_RsaPrivateOperationIpi(u32 RsaParamAddrLow, u32 RsaParamAddrHigh, u32 DstAddrLow,
						u32 DstAddrHigh);
#endif
...
/**************************************************************************************************/
/**
 * @brief	This function handler calls XSecure_RsaInitialize and XSecure_RsaExp server API.
 *
 * @param	RsaParamAddrLow		Lower 32 bit address of the XSecure_RsaInParam structure.
 * @param	RsaParamAddrHigh	Higher 32 bit address of the XSecure_RsaInParam structure.
 * @param	DstAddrLow		Lower 32 bit address of the output data where decrypted data
 * 					to be stored.
 * @param	DstAddrHigh		Higher 32 bit address of the output data where decrypted
 * 					data to be stored.
 *
 * @return
						...
						u32 DstAddrHigh)
{
	...
	Status = XST_FAILURE;
	Status = Xil_SMemSet(RsaOperationParamPtr, sizeof(XSecure_RsaOperationParam), 0U,
			sizeof(XSecure_RsaOperationParam));
	if (Status != XST_SUCCESS) {
	...
	if (RsaKeyParam.OpMode != XSECURE_RSA_CRT_MODE) {
		Status = XST_FAILURE;
		Status = XSecure_MemCpyAndChangeEndianness((UINTPTR)RsaOperationParamPtr->Exp,
				RsaKeyParam.ExpAddr, RsaParams.Size);
		if (Status != XST_SUCCESS) {
			Status = (int)XSECURE_RSA_OP_MEM_CPY_AND_CHANGE_ENDIANNESS_FAILED_ERROR;
			goto END;
		}
		...
		if (RsaKeyParam.OpMode == XSECURE_RSA_EXPOPT_MODE) {
			Status = XST_FAILURE;
			Status = XSecure_MemCpyAndChangeEndianness((UINTPTR)RsaOperationParamPtr->RN,
					RsaKeyParam.RNAddr, RsaParams.Size);
			if (Status != XST_SUCCESS) {
				Status = (int)XSECURE_RSA_OP_MEM_CPY_AND_CHANGE_ENDIANNESS_FAILED_ERROR;
				goto END;
			}
			...
			Status = XST_FAILURE;
			Status = XSecure_MemCpyAndChangeEndianness((UINTPTR)RsaOperationParamPtr->RRN,
					RsaKeyParam.RRNAddr, RsaParams.Size);
			if (Status != XST_SUCCESS) {
				Status = (int)XSECURE_RSA_OP_MEM_CPY_AND_CHANGE_ENDIANNESS_FAILED_ERROR;
				goto END;
			}
		...
		if (RsaKeyParam.IsTotAvail == TRUE) {
			Status = XST_FAILURE;
			Status = XSecure_MemCpyAndChangeEndianness((UINTPTR)RsaOperationParamPtr->Tot,
								RsaKeyParam.TotAddr, RsaParams.Size);
			if (Status != XST_SUCCESS) {
				Status = (int)XSECURE_RSA_OP_MEM_CPY_AND_CHANGE_ENDIANNESS_FAILED_ERROR;
				goto END;
			}
	...
	if ((RsaKeyParam.IsPrimeAvail == TRUE) || (RsaKeyParam.OpMode == XSECURE_RSA_CRT_MODE)) {
		...
		if (RsaKeyParam.OpMode == XSECURE_RSA_CRT_MODE) {
			Status = XST_FAILURE;
			Status = XSecure_MemCpyAndChangeEndianness((UINTPTR)RsaOperationParamPtr->DP,
							RsaKeyParam.DPAddr, RsaKeyParam.PSize);
			if (Status != XST_SUCCESS) {
				Status = (int)XSECURE_RSA_OP_MEM_CPY_AND_CHANGE_ENDIANNESS_FAILED_ERROR;
				goto END;
			}
			...
			Status = XST_FAILURE;
			Status = XSecure_MemCpyAndChangeEndianness((UINTPTR)RsaOperationParamPtr->DQ,
							RsaKeyParam.DQAddr, RsaKeyParam.QSize);
			if (Status != XST_SUCCESS) {
				Status = (int)XSECURE_RSA_OP_MEM_CPY_AND_CHANGE_ENDIANNESS_FAILED_ERROR;
				goto END;
			}
			...
			Status = XST_FAILURE;
			Status = XSecure_MemCpyAndChangeEndianness((UINTPTR)RsaOperationParamPtr->QInv,
							RsaKeyParam.QInvAddr, RsaKeyParam.QSize);
			if (Status != XST_SUCCESS) {
				Status = (int)XSECURE_RSA_OP_MEM_CPY_AND_CHANGE_ENDIANNESS_FAILED_ERROR;
				goto END;
			}
		...
		Status = XST_FAILURE;
		Status = XSecure_MemCpyAndChangeEndianness((UINTPTR)RsaOperationParamPtr->P,
							RsaKeyParam.PAddr, RsaKeyParam.PSize);
		if (Status != XST_SUCCESS) {
			Status = (int)XSECURE_RSA_OP_MEM_CPY_AND_CHANGE_ENDIANNESS_FAILED_ERROR;
			goto END;
		}
		...
		Status = XST_FAILURE;
		Status = XSecure_MemCpyAndChangeEndianness((UINTPTR)RsaOperationParamPtr->Q,
							RsaKeyParam.QAddr, RsaKeyParam.QSize);
		if (Status != XST_SUCCESS) {
			Status = (int)XSECURE_RSA_OP_MEM_CPY_AND_CHANGE_ENDIANNESS_FAILED_ERROR;
			goto END;
		}
	...
	if (RsaKeyParam.IsPubExpAvail == TRUE) {
		Status = XST_FAILURE;
		Status = Xil_SMemSet(&PubModulus, XSECURE_RSA_4096_KEY_SIZE, 0U,
	...
	if (RsaKeyParam.OpMode == XSECURE_RSA_EXPQ_MODE) {
		...
		Status = XSecure_RsaExpopt((unsigned char *)(UINTPTR)RsaOperationParamPtr->InData,
			RsaOperationParamPtr->Exp, RsaOperationParamPtr->Mod,
			RsaOperationParamPtr->RN, RsaOperationParamPtr->RRN, P, Q, PubExponentPtr,
			Tot, (int)(RsaParams.Size * 8U), OutDataPtr);
	}
	if (Status != XST_SUCCESS) {
	...
	XSECURE_TEMPORAL_IMPL(SStatus, SStatusTmp, XPlmi_MemCpy64, DstAddr, (UINTPTR)OutDataPtr,
				RsaParams.Size);
