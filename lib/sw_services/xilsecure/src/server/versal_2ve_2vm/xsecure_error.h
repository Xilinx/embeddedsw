...
/**
 *
 * @file server/versal_2ve_2vm/xsecure_error.h
 *
...
enum {
	...
	XSECURE_SHA_DMA_COMPONENT_IS_NOT_READY,
					/**< 0x0D - Error when DMA component is not ready */

	XSECURE_SHA_DMA_TRANSFER_ERROR,
					/**< 0x0E - Error when SHA padding DMA transfer fails */
	...
	XSECURE_AES_ZERO_PUF_KEY_NOT_ALLOWED,	/**< 0x5B - Error when PUF Key is selected
						as key source and PUF key is zeroized */
	XSECURE_AES_UNALIGNED_SIZE_ERROR,      /**< 0x5C - Error when data is unaligned*/
	XSECURE_AES_INVALID_MODE,	/**< 0x5D - Error when invalid mode is used
					for AES operation */
	XSECURE_INVALID_RESOURCE,	/**< 0X5F - Error when resource is other than SHA/AES */
	XSECURE_AES_DMA_COMPONENT_IS_NOT_READY,	/**< 0X60 - Error when DMA component
						is not ready */
	...
	XSECURE_LMS_OTS_PUB_KEY_SIGN_TYPE_MISMATCH_ERROR,	/**< 0x90 - LMS OTS Signature
	...
	XSECURE_LMS_OTS_SIGN_SHA_DIGEST_FAILED_ERROR,	/**< 0x93 - LMS OTS Signature
				Verification - hash chain per iteration sha digest failed */
	XSECURE_LMS_OTS_SIGN_UNSUPPORTED_TYPE_ERROR,	/**< 0x94 - LMS OTS Signature
				Verification - parameter lookup failed - not supported type */
	XSECURE_AUTH_LMS_OTS_INVALID_SIGN_ADDR_ERROR,	/**< 0x95 - LMS OTS Signature
				Verification - when signature to be used is at invalid address */
	XSECURE_AUTH_LMS_DIGEST_CHECKSUM_FAILED_ERROR,	/**< 0x96 - LMS OTS Signature
				Verification - checksum calculation failed during
				digest calculation */
	XSECURE_LMS_OTS_DIGEST_CHECKSUM_OP_FAILED_ERROR,	/**< 0x97 - LMS OTS Signature
				Verification - Digest calculation operation failed */
	XSECURE_LMS_SIGN_EXPECTED_PUB_KEY_ADDR_ERROR,		/**< 0x98 - LMS Signature
				Verification - invalid address for expected public key
				at Merkle tree root */
	XSECURE_LMS_SIGN_EXPECTED_PUB_KEY_LEN_1_ERROR,		/**< 0x99 - LMS Signature
				Verification - invalid len for expected public key at
				Merkle tree root - less than or equal to 4 */
	XSECURE_LMS_SIGN_EXPECTED_PUB_KEY_LEN_2_ERROR,		/**< 0x9A - LMS Signature
				Verification - invalid len for expected public key at Merkle
				tree root - total less than required by parameters */
	XSECURE_LMS_PUB_KEY_UNSUPPORTED_TYPE_1_ERROR,	/**< 0x9B - LMS Signature Verification
				 - LMS public key parameter pick up failed - un supported type */
	XSECURE_LMS_PUB_KEY_LMS_SIGN_TYPE_MISMATCH_ERROR,	/**< 0x9C - LMS Signature
				Verification - LMS type mismatch between LMS
				signature and public key */
	XSECURE_LMS_OTS_PUB_KEY_LMS_OTS_SIGN_TYPE_MISMATCH_ERROR,/**< 0x9D - LMS OTS type mismatch
						 - between LMS OTS signature and public key */
	XSECURE_LMS_SIGN_LEN_1_ERROR,	/**< 0x9E - LMS Signature Verification
					 - LMS signature length less than or equal 4 bytes */
	XSECURE_LMS_SIGN_LEN_2_ERROR,	/**< 0x9F - LMS Signature Verification - LMS signature
					 length less than required for LMS OTS signature */
	XSECURE_LMS_SIGN_LEN_3_ERROR,	/**< 0xA0 - LMS Signature Verification - LMS signature
				length error - total len less than required by parameters */
	XSECURE_LMS_SIGN_UNSUPPORTED_OTS_TYPE_1_ERROR,	/**< 0xA1 - LMS Signature Verification
				 - LMS OTS parameters look up failed - unsupported type */
	XSECURE_LMS_SIGN_UNSUPPORTED_TYPE_1_ERROR,	/**< 0xA2 - LMS Signature Verification
				 - LMS signature parameters lookup failed - un supported type */
	XSECURE_LMS_SIGN_INVALID_NODE_NUMBER_ERROR,	/**< 0xA3 - LMS Signature Verification
				- invalid node number 'q' in a Merkle tree */
	XSECURE_LMS_SIGN_OTS_OP_ERROR,			/**< 0xA4 - LMS Signature Verification
				- LMS OTS signature verification failed */
	XSECURE_LMS_PUB_KEY_AUTHENTICATION_FAILED_ERROR,	/**< 0xA5 - LMS Signature
				Verification - calculated LMS public key did not match with
				expected - authentication failed */
	XSECURE_LMS_PUB_KEY_AUTHENTICATION_GLITCH_ERROR,	/**< 0xA6 - LMS Signature
				Verification - LMS public key comparison glitch detected -
				authentication failed */
	XSECURE_LMS_SIGN_VERIF_SHA_DIGEST_LEAF_FAILED_ERROR,	/**< 0xA7 - LMS Signature
				Verification - LMS signature to public key - leaf node
				sha digest failed */
	XSECURE_LMS_SIGN_VERIF_SHA_DIGEST_INTR_ODD_FAILED_ERROR,/**< 0xA8 - LMS Signature
				Verification - LMS signature to public key - odd internal
				node sha digest failed */
	XSECURE_LMS_SIGN_VERIF_SHA_DIGEST_INTR_EVEN_FAILED_ERROR,/**< 0xA9 - LMS Signature
				Verification - LMS signature to public key - even internal
				node sha digest failed */
	XSECURE_LMS_SIGN_VERIFY_BH_AND_TYPE_SHA_ALGO_MISMATCH_L0_ERROR,/**< 0xAA - LMS Signature
				Verification - SHA algorithm mismatch between one selected in
				Boot header and LMS & OTS - level 0 */
	XSECURE_LMS_SIGN_VERIFY_BH_AND_TYPE_SHA_ALGO_MISMATCH_L1_ERROR,/**< 0xAB - LMS Signature
				Verification - SHA algorithm mismatch between one selected
				in Boot header and LMS & OTS - level 1 */
	XSECURE_LMS_HSS_PUB_KEY_INVALID_LEN_1_ERROR,	/**< 0xAC - LMS HSS Signature
				verification - HSS public key at an invalid address */
	XSECURE_LMS_HSS_PUB_KEY_INVALID_LEN_2_ERROR,	/**< 0xAD - LMS HSS Signature
				verification - HSS pub key len less than required */
	XSECURE_LMS_HSS_SIGN_LEVEL_UNSUPPORTED_ERROR,	/**< 0xAE - LMS HSS Signature
				verification - only two levels of Merkle trees are supported */
	XSECURE_LMS_HSS_L0_PUB_KEY_LMS_TYPE_UNSUPPORTED_ERROR,	/**< 0xAF - LMS HSS
				Signature verification - HSS pub key's LMS type parameter
				look up failed for level 0 tree */
	XSECURE_LMS_HSS_L1_PUB_KEY_LMS_TYPE_1_UNSUPPORTED_ERROR,	/**< 0xB0 - LMS HSS
				Signature verification - HSS pub key's LMS type parameter look
				up failed for level 1 tree - in HSS init */
	XSECURE_LMS_HSS_L1_PUB_KEY_LMS_TYPE_2_UNSUPPORTED_ERROR,	/**< 0xB1 - LMS HSS
				Signature verification - HSS pub key's LMS type parameter look
				up failed for level 1 tree - in HSS Finish */
	XSECURE_LMS_HSS_L0_PUB_KEY_LMS_OTS_TYPE_UNSUPPORTED_ERROR,	/**< 0xB2 - LMS HSS
				Signature verification - HSS pub key's LMS OTS type parameter
				look up failed for level 0 tree */
	XSECURE_LMS_HSS_L1_PUB_KEY_LMS_OTS_TYPE_UNSUPPORTED_ERROR,	/**< 0xB3 - LMS HSS
				Signature verification - HSS pub key's LMS OTS type parameter look
				up failed for level 1 tree */
	XSECURE_LMS_HSS_SIGN_INVALID_LEN_1_ERROR,	/**< 0xB4 - LMS HSS Signature verification
				 - HSS pub key's LMS OTS type parameter look up for level
				1 tree - temporal glitch detected */
	XSECURE_LMS_HSS_L0_SIGN_INVALID_LEN_2_ERROR,	/**< 0xB5 - LMS HSS Signature verification
				 - HSS signature len does not fit OTS signature for level 0 */
	XSECURE_LMS_HSS_L1_SIGN_INVALID_LEN_2_ERROR,	/**< 0xB6 - LMS HSS Signature verification
				 - HSS signature len does not fit OTS signature for level 1 */
	XSECURE_LMS_HSS_SIGN_PUB_KEY_LEVEL_MISMATCH_ERROR,	/**< 0xB7 - LMS HSS Signature
				verification - HSS pub key & Signature levels mismatch */
	XSECURE_LMS_HSS_L0_PUB_KEY_AUTH_FAILED_ERROR,		/**< 0xB8 - LMS HSS Signature
				verification - Level 0 LMS auth op failed */
	XSECURE_LMS_HSS_L1_PUB_KEY_AUTH_FAILED_ERROR,		/**< 0xB9 - LMS HSS Signature
				verification - Level 1 LMS auth op failed */
	XSECURE_LMS_HSS_OTS_SIGN_INVALID_LEN_1_ERROR,	/**< 0xBA - LMS HSS Signature
				verification - OTS sign at level 1 is less than 4 bytes */
	XSECURE_LMS_PUB_OP_FAILED_ERROR,	/**< 0xBB - LMS HSS Signature verification
				 - authenticated public key for level 1 tree copy failed */
	XSECURE_LMS_PUB_OP_FAILED_1_ERROR,	/**< 0xBC - LMS HSS Signature verification
				 - authenticated public key for level 1 tree copy
				failed - iteration mismatch */
	XSECURE_LMS_OTS_CHECKSUM_BUFF_INVALID_LEN_ERROR,	/**< 0xBD - LMS OTS
				Checksum - buffer for digest is of invalid length (0) */
	XSECURE_LMS_OTS_TYPE_UNSUPPORTED_ERROR,		/**< 0xBE - LMS OTS
							type not supported */
	XSECURE_LMS_OTS_TYPE_LOOKUP_GLITCH_ERROR,	/**< 0xBF - LMS OTS
	...
	XSECURE_ECC_PRVT_KEY_GEN_ERR = 0xD6,
