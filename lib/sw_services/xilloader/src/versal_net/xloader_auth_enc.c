...
int XLoader_ImgHdrTblAuth(XLoader_SecureParams *SecurePtr)
{
	...
	Status = XST_FAILURE;
	Status = XSecure_Sha3Initialize(&Sha3Instance, SecurePtr->PmcDmaInstPtr, XSECURE_SHA3_0_DEVICE_ID);
	if (Status != XST_SUCCESS) {
		...
		u8 *MsgHash, XLoader_RsaKey *Key, u8 *Signature)
{
	...
	Status = XSecure_RsaInitialize(&RsaInstance, (u8 *)Key->PubModulus,
	...
	Status = XSecure_RsaPublicEncrypt(&RsaInstance, Signature,
			...
			XilPdi_MetaHdr *MetaHdr)
{
	...
	Status = XSecure_Sha3Initialize(&Sha3Instance,
			SecurePtr->PmcDmaInstPtr, XSECURE_SHA3_0_DEVICE_ID);
	if (Status != XST_SUCCESS) {
