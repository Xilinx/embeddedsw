...
static u8 XLoader_GetDrvNumSD(u32 DeviceFlags)
{
	...
	if ((DeviceFlags == XLOADER_PDI_SRC_SDLS_B0) ||
		(DeviceFlags == XLOADER_PDI_SRC_SDLS_B0_RAW) ||
		(DeviceFlags == XLOADER_PDI_SRC_SD_B1) ||
		(DeviceFlags == XLOADER_PDI_SRC_SDLS_B1) ||
		...
		/*
		 * For XLOADER_EMMC_BOOT_MODE or XLOADER_EMMC_RAW_BOOT_MODE
		 */
		if (DeviceFlags == XLOADER_PDI_SRC_EMMC) {
...
int XLoader_SdInit(u32 DeviceFlags)
{
	...
	if ((PdiSrc == XLOADER_PDI_SRC_SDLS_B0) ||
...
int XLoader_RawInit(u32 DeviceFlags)
{
	...
	if ((PdiSrc == XLOADER_PDI_SRC_SDLS_B0_RAW) ||
