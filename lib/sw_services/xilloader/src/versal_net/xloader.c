...
#define XPM_NODESUBCL_DEV_PL	(7U)
#endif
/************************** Function Prototypes ******************************/
...
#endif
#ifdef XLOADER_SD_0
	{"SDLS_B0", 0U, XLoader_SdInit, XLoader_SdCopy,
		XLoader_SdRelease}, /* SDLS_B0 - 3U*/
#else
	{NULL, 0U, NULL, NULL, NULL},
#endif
	{NULL, 0U, NULL, NULL, NULL},  /* 4U */
#ifdef XLOADER_SD_0
	{"SD_B1", 0U, XLoader_SdInit, XLoader_SdCopy,
		XLoader_SdRelease}, /* SD_B1 - 5U */
#else
	{NULL, 0U, NULL, NULL, NULL},
#endif
#ifdef XLOADER_SD_1
...
#ifdef XLOADER_SD_0
	{"SDLS_B1", 0U, XLoader_SdInit, XLoader_SdCopy,
		XLoader_SdRelease}, /* SDLS_B1 - 0xEU */
#else
	{NULL, 0U, NULL, NULL, NULL},
#endif
...
#ifdef XLOADER_SD_0
	{"SDLS_B0_RAW", 0U, XLoader_RawInit, XLoader_RawCopy,
		XLoader_RawRelease}, /* SDLS_B0_RAW - 0x12U */
#else
	{NULL, 0U, NULL, NULL, NULL},
#endif
#ifdef XLOADER_SD_0
	{"SD_B1_RAW", 0U, XLoader_RawInit, XLoader_RawCopy,
		XLoader_RawRelease}, /* SD_B1_RAW - 0x13U */
#else
	{NULL, 0U, NULL, NULL, NULL},
#endif
#ifdef XLOADER_SD_1
...
#endif
#ifdef XLOADER_SD_0
	{"SDLS_B1_RAW", 0U, XLoader_RawInit, XLoader_RawCopy,
		XLoader_RawRelease}, /* SDLS_B1_RAW - 0x15U */
#else
	{NULL, 0U, NULL, NULL, NULL},
#endif
#ifdef XLOADER_SD_1
...
static int XLoader_PdiRequestBootDevice(PdiSrc_t DeviceFlags)
{
	...
	case XLOADER_PDI_SRC_QSPI32:
		...
		break;
	case XLOADER_PDI_SRC_SDLS_B0:
	case XLOADER_PDI_SRC_SDLS_B0_RAW:
...
static int XLoader_PdiInit(XilPdi* PdiPtr, PdiSrc_t PdiSrc, u64 PdiAddr)
{
	...
	if (PdiPtr->PdiType == XLOADER_PDI_TYPE_FULL) {
		if (((PdiSrc_t)DeviceFlags == XLOADER_PDI_SRC_SDLS_B0)
		|| ((PdiSrc_t)DeviceFlags == XLOADER_PDI_SRC_SD_B1)
		|| ((PdiSrc_t)DeviceFlags == XLOADER_PDI_SRC_SDLS_B1)
		|| ((PdiSrc_t)DeviceFlags == XLOADER_PDI_SRC_EMMC)) {
			...
			if (SdRawBootVal == XLOADER_SD_RAWBOOT_VAL) {
				if ((PdiSrc_t)DeviceFlags == XLOADER_PDI_SRC_SDLS_B0) {
					PdiPtr->PdiSrc = XLOADER_PDI_SRC_SDLS_B0_RAW;
				}
				else if ((PdiSrc_t)DeviceFlags == XLOADER_PDI_SRC_SD_B1) {
					PdiPtr->PdiSrc = XLOADER_PDI_SRC_SD_B1_RAW;
				}
				else if ((PdiSrc_t)DeviceFlags == XLOADER_PDI_SRC_SDLS_B1) {
					PdiPtr->PdiSrc = XLOADER_PDI_SRC_SDLS_B1_RAW;
				}
				else {
...
static int XLoader_StartImage(XilPdi *PdiPtr)
{
	...
	for (Index = 0U; Index < PdiPtr->NoOfHandoffCpus; Index++) {
		...
		ClusterLockstep = PdiPtr->HandoffParam[Index].CpuSettings &
			XIH_PH_ATTRB_CLUSTER_LOCKSTEP_MASK;
		...
		switch (CpuId)
		{
			case XIH_PH_ATTRB_DSTN_CPU_R52_0:
				...
				Status = XPm_RequestWakeUp(PM_SUBSYS_PMC,
					DeviceId, 1U, HandoffAddr, 0U,
					XPLMI_CMD_SECURE);
				if (Status != XST_SUCCESS) {
					Status = XPlmi_UpdateStatus(
						XLOADER_ERR_WAKEUP_R52_0, Status);
					goto END;
				}
				break;
			...
			case XIH_PH_ATTRB_DSTN_CPU_R52_1:
				...
				if (Status != XST_SUCCESS) {
					Status = XPlmi_UpdateStatus(
						XLOADER_ERR_WAKEUP_R52_1, Status);
					goto END;
				}
				break;
			...
			case XIH_PH_ATTRB_DSTN_CPU_PSM:
				...
				if (Status != XST_SUCCESS) {
					...
					XPlmi_Printf(DEBUG_GENERAL, "Waiting for psmfw is present... timed out\n\r");
					goto END;
				}
				break;
			default:
		...
		{
			case XIH_PH_ATTRB_DSTN_CPU_R52_0:
				if (ClusterId > XIH_PH_ATTRB_DSTN_CLUSTER_1) {
					Status = XPlmi_UpdateStatus(
						XLOADER_ERR_WAKEUP_R52_0, 0U);
					goto END;
				}
				XLoader_Printf(DEBUG_INFO, "Request Cluster %d"
						" R52_0 wakeup\r\n", ClusterId);
				XPlmi_UtilRMW(GET_RPU_CLUSTER_CORE_REG(ClusterId,
					XLOADER_RPU_CORE0, RPU_CLUSTER_CORE_CFG0_OFFSET),
					RPU_CLUSTER_CORE_CFG0_CPU_HALT_MASK,0U);
			...
			case XIH_PH_ATTRB_DSTN_CPU_R52_1:
				...
				XLoader_Printf(DEBUG_INFO, "Request Cluster %d"
						" R52_1 wakeup\r\n", ClusterId);
				XPlmi_UtilRMW(GET_RPU_CLUSTER_CORE_REG(ClusterId,
					XLOADER_RPU_CORE1, RPU_CLUSTER_CORE_CFG0_OFFSET),
					RPU_CLUSTER_CORE_CFG0_CPU_HALT_MASK, 0U);
				break;
			...
			case XIH_PH_ATTRB_DSTN_CPU_A78_0:
				XLoader_Printf(DEBUG_INFO, "Request Cluster %d "
						" A78_0 wakeup\r\n", ClusterId);
				Status = XLoader_A78Config(ClusterId, XLOADER_APU_CORE0,
						HandoffAddr, ClusterLockstep);
				if (Status != XST_SUCCESS) {
					Status = XPlmi_UpdateStatus(
						XLOADER_ERR_WAKEUP_A78_0, 0U);
					goto END;
			...
			case XIH_PH_ATTRB_DSTN_CPU_A78_1:
				XLoader_Printf(DEBUG_INFO, "Request Cluster %d "
						" A78_1 wakeup\r\n", ClusterId);
				Status = XLoader_A78Config(ClusterId, XLOADER_APU_CORE1,
						HandoffAddr, ClusterLockstep);
				if (Status != XST_SUCCESS) {
					Status = XPlmi_UpdateStatus(
						XLOADER_ERR_WAKEUP_A78_1, 0U);
			...
			case XIH_PH_ATTRB_DSTN_CPU_A78_2:
				XLoader_Printf(DEBUG_INFO, "Request Cluster %d "
						" A78_2 wakeup\r\n", ClusterId);
				Status = XLoader_A78Config(ClusterId, XLOADER_APU_CORE2,
						HandoffAddr, ClusterLockstep);
				if (Status != XST_SUCCESS) {
					Status = XPlmi_UpdateStatus(
						XLOADER_ERR_WAKEUP_A78_2, 0U);
			...
			case XIH_PH_ATTRB_DSTN_CPU_A78_3:
				XLoader_Printf(DEBUG_INFO, "Request Cluster %d "
						" A78_3 wakeup\r\n", ClusterId);
				Status = XLoader_A78Config(ClusterId, XLOADER_APU_CORE3,
						HandoffAddr, ClusterLockstep);
				if (Status != XST_SUCCESS) {
					Status = XPlmi_UpdateStatus(
						XLOADER_ERR_WAKEUP_A78_3, 0U);
			...
			case XIH_PH_ATTRB_DSTN_CPU_PSM:
				XLoader_Printf(DEBUG_INFO, "Request PSM wakeup\r\n");
				XPlmi_UtilRMW(CRL_PSM_RST_MODE_ADDR, CRL_PSM_RST_WAKEUP_MASK,
						CRL_PSM_RST_WAKEUP_MASK);
				Status = XPlmi_UtilPollForMask(PSMX_GLOBAL_REG_GLOBAL_CNTRL,
				...
				if (Status != XST_SUCCESS) {
					Status = XPlmi_UpdateStatus(
						XLOADER_ERR_WAKEUP_PSM, Status);
					XPlmi_Printf(DEBUG_GENERAL, "Waiting for psmfw is present... timed out\n\r");
					//goto END; TODO: uncomment once a proper psm flow is available
				}
				XPlmi_LpdInit();
#ifdef PLM_PM_EXCLUDE
#ifdef XPAR_XIPIPSU_0_DEVICE_ID
				Status = XPlmi_IpiInit(NULL);
				if (XST_SUCCESS != Status) {
					XPlmi_Printf(DEBUG_GENERAL, "Error %u in IPI initialization\r\n", Status);
				}
#else
				XPlmi_Printf(DEBUG_GENERAL, "IPI is not enabled in design\r\n");
#endif
#endif
				break;
	...
	if ((SecBootMode == XIH_IHT_ATTR_SBD_SAME) ||
		...
		if (SecBootMode == XIH_IHT_ATTR_SBD_PCIE) {
			...
			switch(SecBootMode)
			{
				...
				#ifdef XLOADER_SD_0
					UPdiSrc = (u32)XLOADER_PDI_SRC_SDLS_B0 |
				...
				#ifdef XLOADER_SD_0
					UPdiSrc = (u32)XLOADER_PDI_SRC_SD_B1 |
						XLOADER_SD_SBD_ADDR_SET_MASK |
						(PdiPtr->MetaHdr.ImgHdrTbl.SBDAddr
						<< XLOADER_SD_SBD_ADDR_SHIFT);
					PdiSrc = (PdiSrc_t)UPdiSrc;
				...
				#ifdef XLOADER_SD_0
					UPdiSrc = (u32)XLOADER_PDI_SRC_SDLS_B1 |
						XLOADER_SD_SBD_ADDR_SET_MASK |
						(PdiPtr->MetaHdr.ImgHdrTbl.SBDAddr
						<< XLOADER_SD_SBD_ADDR_SHIFT);
					PdiSrc = (PdiSrc_t)UPdiSrc;
				...
				case XIH_IHT_ATTR_SBD_SDLS_B0_RAW:
					PdiSrc = XLOADER_PDI_SRC_SDLS_B0_RAW;
					PdiAddr = PdiPtr->MetaHdr.ImgHdrTbl.SBDAddr;
					Status = XST_SUCCESS;
					break;
				case XIH_IHT_ATTR_SBD_SD_B1_RAW:
					PdiSrc = XLOADER_PDI_SRC_SD_B1_RAW;
					PdiAddr = PdiPtr->MetaHdr.ImgHdrTbl.SBDAddr;
					Status = XST_SUCCESS;
					break;
				...
				case XIH_IHT_ATTR_SBD_SDLS_B1_RAW:
					PdiSrc = XLOADER_PDI_SRC_SDLS_B1_RAW;
					PdiAddr = PdiPtr->MetaHdr.ImgHdrTbl.SBDAddr;
					Status = XST_SUCCESS;
					break;
