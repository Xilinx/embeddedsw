...
END:
	...
	ClearStatus |= XPlmi_MemSetBytes(&HashStore, XSECURE_SHA_384_HASH_SIZE_IN_BYTES, 0U, XSECURE_SHA_384_HASH_SIZE_IN_BYTES);
 ...
 * @return
 *		- XST_SUCCESS on success.
 *		- XLOADER_ERR_HWROT_BH_AUTH_NOT_ALLOWED if PPK is programmed and boot header
...
int XLoader_CheckSecureStateAuth(volatile u32* AHWRoT)
{
	...
	IsBhdrAuth = ((XPlmi_In32(XIH_BH_PRAM_ADDR + XIH_BH_IMG_ATTRB_OFFSET) &
				XIH_BH_IMG_ATTRB_BH_AUTH_MASK) >> XIH_BH_IMG_ATTRB_BH_AUTH_SHIFT);
	...
	if ((Status == XST_SUCCESS) || (StatusTmp == XST_SUCCESS)) {
		...
		*AHWRoT = XPLMI_RTCFG_SECURESTATE_AHWROT;
		XPlmi_Printf(DEBUG_PRINT_ALWAYS, "State of Boot(Authentication): Asymmetric HWRoT\r\n");
	} else {
		if ((IsBhdrAuth == XIH_BH_IMG_ATTRB_BH_AUTH_VALUE) ||
			(IsBhdrAuthTmp == XIH_BH_IMG_ATTRB_BH_AUTH_VALUE)) {
			/**
