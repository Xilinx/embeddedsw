...
typedef enum {
	...
	XLOADER_SEC_PRTN_HASH_NOT_PRESENT_IN_IHT_OP_DATA_ERR,
		/**<0x2D Error when partition hash is not present for respective partition */
	XLOADER_SEC_PRTN_HASH_COMPARE_FAIL_ERR,
		/**<0x2E Error when partition hash comparison is failed */
	XLOADER_SEC_AES_AAD_OPERATION_FAILED,
		/**<0x2F - Error in AES AAD Operation */
	XLOADER_SEC_LMS_SIGN_VERIFY_FAIL,
		/**<0x30 - Error in LMS sign verification due to error in SHA operation */
	XLOADER_SEC_LMS_PUBKEY_SIZE_VALIDATE_ERR,
		/**<0x31 - Error in LMS sign verification due to invalid public key size */
} XLoader_SecErrCodes;
