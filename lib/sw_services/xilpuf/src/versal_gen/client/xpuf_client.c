...
int XPuf_ClientInit(XPuf_ClientInstance* const InstancePtr, XMailbox* const MailboxPtr)
{
	...
	/**
	 * - Set XMailbox instance provided by the user to client library instance
	 * by validating whether provided instance is initialized and not NULL.
	 * This XMailbox instance will be used by client library to send requests to PLM.
	 */
	if (InstancePtr != NULL) {
...
int XPuf_Registration(const XPuf_ClientInstance *InstancePtr, const u64 DataAddr)
{
	 ...
	 * via IPI mailbox for other environments.
	 */
...
/*****************************************************************************/
/**
 * @brief       This function sends IPI request for PUF regeneration(on demand / ID only).
 *
 * @param	InstancePtr	Pointer to the client instance
 * @param	DataAddr	Address of the data structure which includes
 * 				options to configure PUF
...
/*****************************************************************************/
/**
 * @brief       This function sends IPI request for clearing PUF ID.
 *
 * @param	InstancePtr	Pointer to the client instance
 *
 * @return
