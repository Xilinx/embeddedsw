...
{
	return ((Xil_In32(addr)) & DDR_PHY_DX0LCDLR3_RDQSD_MASK) >>
			(DDR_PHY_DX0LCDLR3_RDQSD_SHIFT);
}

unsigned int get_rd_qsnd(unsigned int addr)
{
	return ((Xil_In32(addr)) & DDR_PHY_DX0LCDLR4_RDQSND_MASK) >>
			(DDR_PHY_DX0LCDLR4_RDQSND_SHIFT);
}

void find_rd_center(void)
{
	int i;

	for (i = 0; i < ddr_config__lanes; i++) {
		rd_center[i].qsd = get_rd_qsd(LANE0LCDLR3_OFFSET +
				(LANEOFFSET * i));
		rd_center[i].qsnd = get_rd_qsnd(LANE0LCDLR4_OFFSET +
				(LANEOFFSET * i));
	}
}

void reset_rd_center(void)
{
	int i;
...
void set_rd_dqs_delay(int position)
{
	...
	for (i = 0; i < ddr_config__lanes; i++) {
		if (((position > 0) && eye_end[i] == 0) ||
		    ((position < 0) && eye_start[i] == 0)) {
			if ((((int)rd_center[i].qsd + position) >= 0) &&
			    (((int)rd_center[i].qsd + position) <= 511))
				Xil_Out32(LANE0LCDLR3_OFFSET + (LANEOFFSET*i),
						(rd_center[i].qsd) + position);
			asm("dsb sy");
			if ((((int)rd_center[i].qsnd + position) >= 0) &&
			    (((int)rd_center[i].qsnd + position) <= 511))
				Xil_Out32(LANE0LCDLR4_OFFSET + (LANEOFFSET*i),
...
void print_qsd_qsnd(void)
{
	int i;

	xil_printf("Lane(QSD/QSND): ");
	for (i = 0; i < ddr_config__lanes; i++) {
		xil_printf("%d(%d,%d), ", i, get_rd_qsd(LANE0LCDLR3_OFFSET +
			(LANEOFFSET*i)), get_rd_qsnd(LANE0LCDLR4_OFFSET +
			(LANEOFFSET*i)));
	}
	xil_printf("\r\n");
}
...
void measure_rd_eye(unsigned long int testaddr, unsigned int len,
		unsigned int pattern, unsigned int num_iters)
{
	...
	Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_SYNC_INT,
			(Xil_ExceptionHandler)eyescan_rdsyncaborthandler,
			(void *) 0);
	Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_SERROR_ABORT_INT,
			(Xil_ExceptionHandler)eyescan_rdserroraborthandler,
			(void *) 0);

	printf("\nRunning Read Eye Tests\n");

	/* Dynamically allocate space for the following arrays and clear them */
	rd_center = (struct RD_Center *) malloc(sizeof(struct RD_Center) *
					ddr_config__lanes);
	if (!rd_center) {
		xil_printf("Not enough memory to run read eye measurement\r\n");
		return;
	}
	...
	for (j = 0; j < num_iters; j++) {
		...
		while (!done) {
			...
			for (i = 0; i < ddr_config__lanes; i++) {
				if (!eye_end[i] &&
				    (((rd_center[i].qsd + position) < 511) ||
				((rd_center[i].qsnd + position) < 511))) {
					done = 0;
				}
			}
	...
	for (i = 0; i < ddr_config__lanes; i++) {
		calc_per_tap_delay(i);
		printf("  %2.2f |", (float) (tap_ps *
				(eye_end[i] - eye_start[i])));
	}
	printf("\n");
	print_line3();
	printf(" EYE WIDTH (%%):\n");
	...
	print_line3();
	for (i = 0; i < ddr_config__lanes; i++) {
		printf("   %2d,%2d |", ((rd_center[i].qsd + eye_end[i]) +
				(rd_center[i].qsd + eye_start[i])) / 2,
				((rd_center[i].qsnd + eye_end[i]) +
				(rd_center[i].qsnd + eye_start[i]))/2);
	}
	printf("\n");
	print_line3();
	reset_rd_center();
