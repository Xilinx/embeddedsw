...
{
	return (Xil_In32(addr) & DDR_PHY_DX0LCDLR1_WDQD_MASK) >>
			(DDR_PHY_DX0LCDLR1_WDQD_SHIFT);
}
...
{
	return (Xil_In32(addr) & DDR_PHY_DX0GTR0_WDQSL_MASK) >>
			(DDR_PHY_DX0GTR0_WDQSL_SHIFT);
}
...
{
	return (Xil_In32(addr) & DDR_PHY_DX0MDLR0_IPRD_MASK) >>
			(DDR_PHY_DX0MDLR0_IPRD_SHIFT);
}
...
{
	return (Xil_In32(addr) & DDR_PHY_DX0MDLR0_TPRD_MASK) >>
			(DDR_PHY_DX0MDLR0_TPRD_SHIFT);
}
...
{
	return (Xil_In32(addr) & DDR_PHY_DX0GTR0_WLSL_MASK) >>
			(DDR_PHY_DX0GTR0_WLSL_SHIFT);
}
...
{
	return (Xil_In32(addr) & DDR_PHY_DX0LCDLR0_WLD_MASK) >>
			(DDR_PHY_DX0LCDLR0_WLD_SHIFT);
}
...
{
	return (Xil_In32(addr) & DDR_PHY_DX0GTR0_DGSL_MASK) >>
			(DDR_PHY_DX0GTR0_DGSL_SHIFT);
}
...
void force_reinit(void)
{
	...
	rdval = Xil_In32(DDR_PHY_PIR);
	wrval = rdval | (1 << DDR_PHY_PIR_INIT_SHIFT);
	Xil_Out32(DDR_PHY_PIR, wrval);
	asm("dsb sy");
}
...
void poll_training_done(void)
{
	...
	unsigned int redone = 0;


	while (!idone && !redone) {
		rdval = Xil_In32(DDR_PHY_PGSR0);
...
void get_wdqd_sw_workaround(void)
{
	int i;
	unsigned int init_iprd;
	unsigned int iprd;

	wr_ds = (struct WR_DS *) malloc(sizeof(struct WR_DS) *
			ddr_config__lanes);
	if (!wr_ds)
	...
	for (i = 0; i < ddr_config__lanes; i++)
		xil_printf("L%d(%u/%u), ", i, wr_center[i].wdqd,
				wr_center[i].wdqsl);
	xil_printf("\r\n");
	#endif
...
void measure_wr_eye(unsigned long int testaddr, unsigned int len,
		unsigned int pattern, unsigned int num_iters)
{
	int i = 0;
	int done = 0;
	int position = 0;
	...
	Xil_ExceptionHandler SyncHandler = NULL;
	Xil_ExceptionHandler SerrorHandler = NULL;
	void *syncdata = NULL;
	void *serrordata = NULL;
	...
	Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_SYNC_INT,
			(Xil_ExceptionHandler)eyescan_wrsyncaborthandler,
			(void *) 0);
	Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_SERROR_ABORT_INT,
			(Xil_ExceptionHandler)eyescan_wrserroraborthandler,
			(void *) 0);

	printf("\nRunning Write Eye Tests\n");

	wr_center = (struct WR_Center *) malloc(sizeof(struct WR_Center) *
					ddr_config__lanes);
	if (!wr_center)
		xil_printf("Not enough memory to run write eye measurement\n");
	tap_count = (unsigned int *) malloc(sizeof(unsigned int) *
					ddr_config__lanes);
	if (!tap_count)
		xil_printf("Not enough memory to run write eye measurement\n");
	raw_centers = (unsigned int *) malloc(sizeof(unsigned int) *
					ddr_config__lanes);
	if (!raw_centers)
		xil_printf("Not enough memory to run write eye measurement\n");
	...
	for (j = 0; j < num_iters; j++) {
		...
		while (!done) {
			...
			if (ddr_config__lanes == 8)
				...
				xil_printf("DDR Config Failure\r\n");

			print_results();

			for (i = 0; i < ddr_config__lanes; i++) {
				if (Xil_In32(RESULTS_BASE+(i*4)) != 0 &&
						eye_end_temp[i] == 0)
	...
	print_line3();
	for (i = 0; i < ddr_config__lanes; i++) {
		printf("   %1.2f  |", (float)((1000000 / (ddr_freq * 4)) /
				(float) tap_count[i]));
	}
	printf("\n");
	print_line3();
	...
	for (i = 0; i < ddr_config__lanes; i++) {
		...
		printf(" %2.2f  |", (float) (tap_ps * (eye_end[i] -
				eye_start[i])));
	}
	printf("\n");
	print_line3();
	...
	for (i = 0; i < ddr_config__lanes; i++) {
		calc_per_tap_delay(i);
		printf("  %2.2f  |", 100 * (tap_ps * (eye_end[i] -
				eye_start[i])) / (1000000 / (ddr_freq * 4)));
	}
	printf("\n");
	print_line3();
	printf(" EYE CENTER:\n");
	print_line3();
	for (i = 0; i < ddr_config__lanes; i++) {
		raw_centers[i] = (wr_center[i].wdqsl * tap_count[i]) +
				wr_center[i].wdqd;
		raw_centers[i] = ((raw_centers[i] + eye_start[i]) +
				(raw_centers[i] + eye_end[i])) / 2;
		printf("  %2d,%2d  |", raw_centers[i] / tap_count[i],
				raw_centers[i] % tap_count[i]);
	}
	printf("\n");
	print_line3();
