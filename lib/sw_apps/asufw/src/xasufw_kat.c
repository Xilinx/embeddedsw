...
#define XASUFW_LMS_KAT_MSG_SIZE			(128U)	/**< LMS KAT message size */
#define XASUFW_LMS_KAT_SIG_SIZE			(1292U)	/**< LMS KAT signature size for H=5, w=4 */
#define XASUFW_LMS_KAT_PUB_KEY_SIZE		(56U)	/**< LMS KAT public key size */
...
static const u8 LmsKatMessage[XASUFW_LMS_KAT_MSG_SIZE] = {
...
/* LMS KAT Public Key (56 bytes) - LMS_SHA256_M32_H5, LMOTS_SHA256_N32_W4 */
static const u8 LmsKatPublicKey[XASUFW_LMS_KAT_PUB_KEY_SIZE] = {
...
/* LMS KAT Signature (1292 bytes) - For H=5, w=4 */
static const u8 LmsKatSignature[XASUFW_LMS_KAT_SIG_SIZE] = {
...
/*************************************************************************************************/
/**
 * @brief	This function runs LMS signature verification KAT.
 *
 ...
 *	- XASUFW_LMS_KAT_FAILED, if LMS KAT fails.
 *
 *************************************************************************************************/
s32 XAsufw_LmsOperationKat(XAsufw_Dma *AsuDmaPtr)
{
	s32 Status = XASUFW_FAILURE;
	XSha *ShaInstPtr = XSha_GetInstance(XASU_XSHA_0_DEVICE_ID);
	...
	LmsParams.MsgLen = XASUFW_LMS_KAT_MSG_SIZE;
	LmsParams.SignatureLen = XASUFW_LMS_KAT_SIG_SIZE;
	LmsParams.PublicKeyLen = XASUFW_LMS_KAT_PUB_KEY_SIZE;
	...
	Status = XLms_SignatureVerification(ShaInstPtr, AsuDmaPtr, &LmsParams);
	if (Status != XASUFW_SUCCESS) {
		Status = XAsufw_UpdateErrorStatus(Status, XASUFW_LMS_KAT_FAILED);
	}
