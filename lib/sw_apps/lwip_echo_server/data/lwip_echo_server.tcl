...
proc swapp_is_supported_sw {} {
	...
	if {$processor_type in {"psv_cortexa72" "psx_cortexa78"}} {
		set procdrv [hsi::get_sw_processor]
		set compiler [::common::get_property CONFIG.compiler $procdrv]
		if {[string compare -nocase $compiler "arm-none-eabi-gcc"] == 0} {
			error "ERROR: lwip library does not support 32 bit A53/A78 compiler";
		return;
            }
	}
...
proc generate_emac_config {fp} {
    ...
    set temacs_ps7 [hsi::get_cells -hier -filter { ip_name == "ps7_ethernet" }];
    set temacs_psu [hsi::get_cells -hier -filter { ip_name == "psu_ethernet" }];
    set temacs_psv [hsi::get_cells -hier -filter { ip_name == "psv_ethernet" }];
    set temacs_psx [hsi::get_cells -hier -filter { ip_name == "psx_ethernet" }];

    if {([llength $temacs_ps7] > 0) || ([llength $temacs_psu] > 0) ||
	([llength $temacs_psv] > 0) || ([llength $temacs_psx] > 0)} {
