...
static u8 QspiMode;
static XPlm_BootModes QspiBootMode;
static u8 QspiProto;
 ...
 * 			- XPLM_ERR_UNSUPPORTED_QSPI_FLASH_ID on unsupported QSPI flash.
 ...
 * 			and driver.
 *
 * @param	Mode is unused and is only for compatibility with other
...
static u32 XPlm_QspiGetBusWidth(void)
{
	...
	// TODO Add macronix 24bit read command logic
...
static u32 SendBankSelect(u32 BankSel)
{
	 ...
	 */
	if ((QspiFlashMake == XPLM_MICRON_ID) ||
		(QspiFlashMake == XPLM_MACRONIX_ID)) {
		/**
...
u32 XPlm_QspiCopy(u64 SrcAddr, u32 DestAddr, u32 Length, u32 Flags)
{
	...
	XPlm_Printf(DEBUG_INFO, "QSPI Reading Src 0x%08x, Dest 0x%08x, "
	...
	while (RemainingBytes > 0U) {
		...
		XPlm_Printf(DEBUG_DETAILED, "QSPI Read Src 0x%08x, "
				"Dest 0x%08x, Length %08x\r\n",
				SrcAddrLow, (u32)(DestAddr + DestOffset),
				TransferBytes);
