...
int flush;
{
	...
	for (;;)
		...
		case LENLENS:
			...
			state->lenbits = 7;
			ret =
			    inflate_table(CODES, state->lens, 19,
		...
		case CODELENS:
			...
			state->distbits = 6;
			ret =
			    inflate_table(DISTS, state->lens + state->nlen,
			...
			if (here.op && (here.op & 0xf0) == 0) {
				...
				for (;;) {
					here = state->lencode[last.val +
								...
								op) >> last.
							       bits)];
					if ((unsigned)(last.bits + here.bits) <=
					    bits)
						break;
					PULLBYTE();
				}
